...
trait JdbcStatementBuilderComponent { self: JdbcProfile =>
  ...
  class QueryBuilder(val tree: Node, val state: CompilerState) {
    ...
    protected def buildComprehension(c: Comprehension): Unit = {
      ...
      def containsSymbolInSubquery(s: TermSymbol) =
        c.children.iterator
          .drop(1)
          .flatMap(
            _.collect { case c: Comprehension => c }.toSeq
    ...
    protected def flattenJoins(
        s: TermSymbol,
        n: Node
    ): (Seq[(TermSymbol, Node)], Seq[Node]) = {
      def f(
          s: TermSymbol,
          n: Node
      ): Option[(Seq[(TermSymbol, Node)], Seq[Node])] = n match {
      ...
      case n: Apply =>
        n match {
          ...
          case Library.Database()
              if !capabilities
                .contains(RelationalCapabilities.functionDatabase) =>
            b += "''"

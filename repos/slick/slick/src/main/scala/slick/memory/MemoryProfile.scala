...
trait MemoryProfile extends RelationalProfile with MemoryQueryingProfile {
  ...
  class SchemaActionExtensionMethodsImpl(schema: SchemaDescription)
    ...
    def create = dbAction { session =>
      tables.foreach(t =>
        session.database.createTable(t.tableName, t.create_*.map { fs =>
          new HeapBackend.Column(fs, typeInfoFor(fs.tpe))
        }.toIndexedSeq, t.indexes.toIndexedSeq, t.tableConstraints.toIndexedSeq)
      )
    }

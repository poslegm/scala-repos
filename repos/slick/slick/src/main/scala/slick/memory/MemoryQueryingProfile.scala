...
trait MemoryQueryingProfile extends BasicProfile {
  ...
  class MemoryCodeGen
    ...
    override def compile(
        n: Node
    ): ResultConverter[MemoryResultConverterDomain, _] = n match {
      ...
      case Library.SilentCast(
            sel @ Select(_, ElementSymbol(idx)) :@ OptionType(tpe2)
          ) :@ tpe if !tpe.isInstanceOf[OptionType] =>
        val base =
  ...
  object ProductOfCommonPaths {
    def unapply(
        n: ProductNode
    ): Option[(TermSymbol, Vector[List[TermSymbol]])] =

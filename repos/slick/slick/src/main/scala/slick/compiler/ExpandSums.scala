...
class ExpandSums extends Phase {
  ...
  def expandSums(n: Node): Node = {
    ...
    def tr(
        tree: Node,
        oldDiscCandidates: Set[(TypeSymbol, List[TermSymbol])]
    ): Node = {
      val discCandidates =
        oldDiscCandidates ++
          (tree match {
  ...
  def translateJoin(
      bind: Bind,
      discCandidates: Set[(TypeSymbol, List[TermSymbol])]
  ): Bind = {
    ...
    val ref = ProductNode(
      ConstArray(optionCast(0, ldisc), optionCast(1, rdisc))
    ).infer()
  ...
  def collectDiscriminatorCandidates(
      n: Node
  ): Set[(TypeSymbol, List[TermSymbol])] =
  ...
  def expandConditionals(n: Node): Node = {
    ...
    def tr(n: Node): Node =
      n.mapChildren(tr, keepType = true) match {
        ...
        case cast @ Library
              .SilentCast(ch) :@ Type.Structural(ProductType(typeCh)) =>
          invalidate(ch)

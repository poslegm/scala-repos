...
class NestingTest extends AsyncTest[RelationalTestDB] {
  ...
  def testNestedOptions = {
    ...
    val q1d2t: Query[Rep[
      Option[(Rep[Int], Rep[String], Rep[Option[Int]])]
    ], _, Seq] = q1d2
    ...
    lazy val t5 = seq(
      mark("q1e1", q1e1.result).map(
        _ shouldBe r
          .map(t => Some(t))
          .map { to => to.flatMap { t => Some(t._2) } }
      ),
      mark("q1e2", q1e2.result).map(
        _ shouldBe r
          .map(t => Some(t))
          .map { to => to.flatMap { t => t._3 } }
      ),

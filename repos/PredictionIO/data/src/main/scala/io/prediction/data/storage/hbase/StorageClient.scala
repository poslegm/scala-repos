...
class StorageClient(val config: StorageClientConfig)
  ...
  try {
    ...
    case e: MasterNotRunningException =>
      error(
        "HBase master is not running (ZooKeeper ensemble: " + conf
    ...
    case e: ZooKeeperConnectionException =>
      error(
        "Cannot connect to ZooKeeper (ZooKeeper ensemble: " + conf
          .get("hbase.zookeeper.quorum") + "). Please make sure that the " +

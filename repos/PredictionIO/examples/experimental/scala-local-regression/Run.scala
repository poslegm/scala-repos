...
case class LocalDataSource(val dsp: DataSourceParams)
    extends LDataSource[DataSourceParams, String, TrainingData, Vector[Double], Double] {
  override def read(
...
case class LocalAlgorithm()
    extends LAlgorithm[EmptyParams, TrainingData, Array[Double], Vector[Double], Double] {

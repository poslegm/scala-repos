...
case class ParallelDataSource(val dsp: DataSourceParams)
    extends PDataSource[DataSourceParams, Integer, RDD[LabeledPoint], Vector, Double] {
  override def read(
      sc: SparkContext
...
case class ParallelSGDAlgorithm(val ap: AlgorithmParams)
    extends P2LAlgorithm[AlgorithmParams, RDD[LabeledPoint], RegressionModel, Vector, Double] {

...
object Console extends Logging {
    ...
    val parser = new scopt.OptionParser[ConsoleArgs]("pio") {
      ...
      cmd("eval")
        ...
        (arg[String]("<evaluation-class>") action { (x, c) =>
          ...
          "[<engine-parameters-generator-class>]"
        ) optional () action { (x, c) =>
      ...
      cmd("app").text("Manage apps.\n").action { (_, c) =>
        ...
        (cmd("new").text("Create a new app key to app ID mapping.").action {
          ...
          .text("List all apps.")
          .action { (_, c) => c.copy(commands = c.commands :+ "list") }, note(
          ""
        ...
        note(""), cmd("data-delete").text("Delete data of an app").action {
          (_, c) => c.copy(commands = c.commands :+ "data-delete")
        } children
          (arg[String]("<name>") action { (x, c) =>
            c.copy(app = c.app.copy(name = x))
      ...
      cmd("accesskey").text("Manage app access keys.\n").action { (_, c) =>
        ...
        (cmd("new").text("Add allowed event(s) to an access key.").action {
          ...
          (_, c) => c.copy(commands = c.commands :+ "list")
        } children
  ...
  def train(ca: ConsoleArgs): Int = {
    ...
    withRegisteredManifest(
      ...
      ca.common.engineId,
      ca.common.engineVersion
    ) { em => RunWorkflow.newRunWorkflow(ca, em) }
  }
  ...
  def deploy(ca: ConsoleArgs): Int = {
    ...
    withRegisteredManifest(
      ...
      val engineInstance =
        ca.engineInstanceId map { eid => engineInstances.get(eid) } getOrElse {

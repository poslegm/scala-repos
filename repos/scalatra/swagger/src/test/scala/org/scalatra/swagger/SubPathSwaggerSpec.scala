...
class ApiController()(implicit val swagger: Swagger)
  ...
  val listHackers =
    (apiOperation[List[Hacker]](
      "listHackers"
    ) summary "Show all hackers" notes "Shows all available hackers.")
  ...
  val getHacker =
    (apiOperation[Hacker](
      "getHacker"
    ) summary "Retrieve a single hacker by id" notes "Foo" parameters Parameter(
      "id",
      DataType.Int,
  ...
  val createHacker =
    (apiOperation[Hacker](
      "createHacker"
    ) summary "Create a new hacker" notes "firstname, lastname, motto, and year of birth are required" parameters
      (Parameter(
        "firstname",
        DataType.String,

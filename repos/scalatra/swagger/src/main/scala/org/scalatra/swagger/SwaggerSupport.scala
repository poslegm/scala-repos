...
trait SwaggerSupportSyntax extends Initializable with CorsSupport {
  ...
  protected def apiOperation[T: Manifest: NotNothing](
      nickname: String
  ): SwaggerOperationBuilder[_ <: SwaggerOperation]
...
trait SwaggerSupport
  ...
  protected implicit def operationBuilder2operation[T](
      bldr: SwaggerOperationBuilder[Operation]
  ...
  protected def extractOperation(
    ...
    op map (_.copy(method = method)) getOrElse {
      ...
      val produces =
        route.metadata.get(Symbols.Produces) map (_.asInstanceOf[
          List[String]
        ]) getOrElse Nil
      val consumes =
        route.metadata.get(Symbols.Consumes) map (_.asInstanceOf[
          List[String]
        ]) getOrElse Nil
      Operation(
        method = method,

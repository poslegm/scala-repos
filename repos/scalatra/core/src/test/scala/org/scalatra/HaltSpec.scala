...
class HaltSpec extends ScalatraSpec {
  def is =
    "halt with no arguments should" ^ "behave like a common halt" ^ commonHalt(
      "/no-args"
    ) ^ "not alter the status" ! status(
      "/no-args",
      501
    ) ^ "retain the headers" ! retainsHeaders(
      "/no-args"
      ...
      "/status-and-reason",
      403
    ) ^ end ^ "halt with only a body should" ^ "behave like a common halt" ^ commonHalt(
      "/body"
    ) ^ "not alter the status" ! status(
      "/body",
      501
  ...
  def commonHalt(uri: String) =
    "not execute the rest of the action" ! haltsAction(
      uri
    ) ^ "retain the headers" ! retainsHeaders(
      uri

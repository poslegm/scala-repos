...
class BasicFieldDescriptor[T](
  ...
  def validateWith(
      bindingValidators: BindingValidator[T]*
  ): FieldDescriptor[T] = {
...
trait DataboundFieldDescriptor[S, T] extends FieldDescriptor[T] {
  ...
  def apply[V](original: Either[String, Option[V]])(implicit
      mv: Manifest[V],
      convert: TypeConverter[V, T]
  ): DataboundFieldDescriptor[V, T] =
...
object BindingValidators {
  ...
  class ValidatableStringBinding(b: FieldDescriptor[String]) {
    ...
    def notBlank(
        messageFormat: String = b.requiredError
    ...
    def validEmail(
        messageFormat: String = "%s must be a valid email address."
    ): FieldDescriptor[String] =

...
object Test {
  ...
  (
      (x: Int) => 0
  ): NonClassType // "class type required". I think we should avoid SAM translation here.
  ((x: Int) => 0): NoEmptyConstructor
  ((x: Int) => 0): OneEmptyConstructor // okay
  (
      (x: Int) => 0
  ): OneEmptySecondaryConstructor // derived class must have an empty *primary* to call.

...
class FutureTests extends MinimalScalaTest {
  ...
  "Futures" should {
    ...
    "have proper const representation for failure" in {
      ...
      assert(
        ECNotUsed(ec => f.map(_ => fail("map should not have been called"))(ec)) eq f
      )
      assert(
        ECNotUsed(ec =>
          f.flatMap(_ => fail("flatMap should not have been called"))(ec)
        ) eq f
      )
      assert(
        ECNotUsed(ec =>
          f.filter(_ => fail("filter should not have been called"))(ec)
        ) eq f
      )
      assert(
        ECNotUsed(ec =>
      ...
      assert(
        ECNotUsed(ec =>
          f.zipWith(f)({ (_, _) => fail("zipWith should not have been called") }
          )(ec)
        ) eq f
      )
    }
    ...
    "have a never member representing a never completed Future of Nothing" in {
      ...
      ECNotUsed(ec =>
        test.transformWith(_ =>
          fail("transformWith should not have been called")
        )(ec) eq test
      )
  ...
  "A future with global ExecutionContext" should {
    ...
    "fold" in {
      ...
      val futures =
        (0 to 9) map { idx => async(idx, idx * 20) }

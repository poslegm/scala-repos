...
class SetMapConsistencyTest {
      ...
      if (g1 != g2) {
        ...
        throw new Exception(
          s"Disagreement after ${what.result} between ${map1.title} and ${map2.title} on get of ${keys(
            j
          )} (#$j) on step $i: $g1 != $g2 using methods $gn1 and $gn2 resp.; in full\n$map1\n$map2\n$temp"
        )
      }
    }
    true
  ...
  def churnIntMaps() {
    ...
    assert(
      maps
        .sliding(2)
        .forall { ms => churn(ms(0)(), ms(1)(), intKeys, 2000) }
    )
  }
  ...
  def churnLongMaps() {
    ...
    assert(
      maps
        .sliding(2)
        .forall { ms => churn(ms(0)(), ms(1)(), longKeys, 10000) }
    )
  }
  ...
  def churnStringMaps() {
    ...
    assert(
      maps
        .sliding(2)
        .forall { ms => churn(ms(0)(), ms(1)(), stringKeys, 5000) }
    )
  }
  ...
  def churnAnyMaps() {
    ...
    assert(
      maps
        .sliding(2)
        .forall { ms => churn(ms(0)(), ms(1)(), anyKeys, 10000) }
    )
  }

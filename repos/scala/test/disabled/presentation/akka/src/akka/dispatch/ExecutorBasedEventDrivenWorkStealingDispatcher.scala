...
class ExecutorBasedEventDrivenWorkStealingDispatcher(
  ...
  override private[akka] def reRegisterForExecution(
    ...
    try {
      ...
      while (donateFrom(
               mbox
             )) {} //When we reregister, first donate messages to another actor
  ...
  protected def donateFrom(
    ...
    doFindDonorRecipient(
      donorMbox,
      actors,
      (System.currentTimeMillis % actors.size).asInstanceOf[Int]
    ) match {
      case null      => false
  ...
  protected def attemptDonationOf(
    ...
    try {
      ...
      doFindDonorRecipient(
        donorMbox,
        actors,
        System.identityHashCode(message) % actors.size
      ) match {
        case null      => false

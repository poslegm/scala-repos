  ...
  def scalaParser(entryPoint: String, globalBase: File): Parser[String] = {
    ...
    val UniversalOpt =
      Property | oneOf(
        pathSettingNames.map(PathSetting) ++ phaseSettings.map(
          PhaseSettingParser
        ) ++ booleanSettingNames.map(BooleanSetting) ++ stringSettingNames
    ...
    val ScalaDocExtraSettings = oneOf(
      scalaDocBooleanSettingNames.map(BooleanSetting) ++ scalaDocIntSettingNames
        ...
        case (k, v) => ChoiceSetting(k, v)
      } ++ scaladocStringSettingNames.map(
        StringSetting
      ) ++ scaladocPathSettingNames
    ...
    val P = entryPoint match {
      case "scala" =>
        ...
        Opt(
          Space ~>
            (options | concat(
              concat(options ~ Space.string) ~ runnableAndArgs
            ) | runnableAndArgs)
        )
      case "scaladoc" =>

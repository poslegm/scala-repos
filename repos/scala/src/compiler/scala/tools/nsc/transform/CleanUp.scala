    ...
    def transformApplyDynamic(ad: ApplyDynamic) = {
      ...
      def reflectiveMethodCache(
        ...
        addStaticMethodToClass((reflMethodSym, forReceiverSym) => {
          val methodCache =
            reflMethodSym.newVariable(
              mkTerm("methodCache"),
              ad.pos
            ) setInfo StructuralCallSite.tpe
          val methodSym =
            reflMethodSym
      ...
      def callAsReflective(paramTypes: List[Type], resType: Type): Tree = {
        ...
        gen.evalOnce(qual, currentOwner, unit) { qual1 =>
          ...
          def genDefaultCall = {
            ...
            val invokeExc =
              currentOwner.newValue(
                mkTerm(""),
                ad.pos
              ) setInfo InvocationTargetExceptionClass.tpe
            def catchVar =
              Bind(
                invokeExc,
                Typed(
          ...
          def genArrayCallWithTest =
            IF(
              (qual1() GETCLASS ()) DOT nme.isArray
            ) THEN genArrayCall ELSE genDefaultCall

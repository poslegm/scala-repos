  ...
  def minimizeParents(parents: List[Type]): List[Type] =
    if (parents.isEmpty) parents
      ...
      while (rest.nonEmpty) {
        ...
        val nonLeaf =
          leaves exists { t => t.typeSymbol isSubClass candidate.typeSymbol }
        if (!nonLeaf) {
...
//                }
            ...
            if (isPrimitiveValueMember(tree.symbol) &&
              ...
              val applied =
                Apply(
                  qual1,
                  List()
                ) setPos qual1.pos setType qual1.tpe.resultType
  ...
  class ErasureTransformer(unit: CompilationUnit) extends Transformer {
    ...
    private def bridgeDefs(
        owner: Symbol
    ): (List[Tree], immutable.Set[Symbol]) = {
      assert(phase == currentRun.erasurePhase, phase)
              ...
              def mkIsInstanceOf(q: () => Tree)(tp: Type): Tree =
                Apply(
                  TypeApply(
                    ...
                    List(TypeTree(tp) setPos targ.pos)
                  ) setPos fn.pos,
                  List()
                ) setPos tree.pos
              targ.tpe match {

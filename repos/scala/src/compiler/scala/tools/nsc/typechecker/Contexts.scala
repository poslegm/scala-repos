...
trait Contexts { self: Analyzer =>
  ...
  private lazy val startContext = NoContext.make(
    Template(
      ...
      noSelfType,
      List()
    ) setSymbol global.NoSymbol setType global.NoType,
  ...
  class Context private[typechecker] (
    ...
    def savingUndeterminedTypeParams[A](
        reportAmbiguous: Boolean = ambiguousErrors
    )(body: => A): A = {
      withMode() {
    ...
    private def resolveAmbiguousImport(
      ...
      else (// The symbol names are checked rather than the symbols themselves because
  ...
  abstract class ContextReporter(
    ...
    @inline final def propagatingErrorsTo[T](
        target: ContextReporter
    )(expr: => T): T = {
      val res =
    ...
    final private[typechecker] def ++=(
        errors: Traversable[AbsTypeError]
    ): Unit = errorBuffer ++= errors
    ...
    private def recordUsage(sel: ImportSelector, result: Symbol): Unit = {
      debuglog(
        s"In $this at ${pos.source.file.name}:${posOf(sel).line}, selector '${selectorString(
          sel
        )}' resolved to ${if (tree.symbol.hasCompleteInfo) s"(qual=$qual, $result)"

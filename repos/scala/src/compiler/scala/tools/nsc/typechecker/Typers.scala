...
trait Typers
  ...
  abstract class Typer(context0: Context)
    ...
    @inline
    final def typerWithCondLocalContext[T](
        c: => Context
    )(cond: Boolean)(f: Typer => T): T =
            ...
            val tree1 =
              (if (tree.isType) tree
                 ...
                 TypeApply(
                   tree,
                   tparams1 map
                     (tparam => TypeTree(tparam.tpeHK) setPos tree.pos.focus)
                 ) setPos tree.pos)
    ...
    def adaptToMemberWithArgs(
      ...
      silent(
        _.adaptToMember(qual, HasMember(name), reportAmbiguous = false)
      ) orElse
        (errs =>
          onError {
    ...
    /** Typechecks the mishmash of trees that happen to be stuffed into the primary constructor of a given template.
      ...
      */
    private def typedPrimaryConstrBody(
        templ: Template
                ...
                if (args1 exists { _.isErrorTyped }) duplErrTree
                  ...
                  debuglog(
                    "infer method inst " + fun + ", tparams = " + tparams +
                      ...
                      ", parambounds = " + tparams.map(_.info)
                  ) //debug
      ...
      def typedSelectInternal(tree: Tree, qual: Tree, name: Name): Tree = {
        ...
        if (!reallyExists(sym)) {
          def handleMissing: Tree = {
            ...
            def asTypeSelection =
              (if (context.unit.isJava && name.isTypeName) {
                 ...
                 atPos(tree.pos)(
                   gen.convertToSelectFromType(qual, name)
                 ) match {
                   case EmptyTree => None
          ...
          val (result, accessibleError) = silent(
            _.makeAccessible(tree1, sym, qual.tpe, qual)
          ) match {
          ...
          def maybeWarn(s: String): Unit = {
            ...
            if (suspiciousExpr.nonEmpty)
              ...
              suspiciousIdents find isPlausible foreach
                (sym =>
                  warn(s"detected interpolated identifier `$$${sym.name}`")
                ) // "$id"
          }
          lit match {

...
object scaladoc extends Command {
  ...
  val name = Section(
    ...
    MBold(
      command
    ) & " " & NDash & " Documentation generator for the " & scalaLink & " language"
  )
  ...
  val description = Section(
    ...
    "The " & MBold(
      command
    ) & " tool reads class and object definitions, " & "written in the " & scalaLink & " programming language, and generates " & "their API as HTML files.",
    "By default, the generator puts each HTML file in the same directory as " & "its source file. You can specify a separate destination directory with " & CmdOption(
      "d"
    ) & "(see " & Link(Bold("OPTIONS"), "#options") & ", below).",
        ...
        Definition(
          ...
          SeqPara(
            "Indicate whether source should be compiled with deprecation " & "information; defaults to " & Mono(
              "off"
            ) & " (" & "accepted values are: " & Mono("on") & ", " & Mono(
              "off"
  ...
  val seeAlso = Section(
    ...
    Link(Bold("fsc") & "(1)", "fsc.html") & ", " & Link(
      Bold("scala") & "(1)",
      "scala.html"
    ) & ", " & Link(Bold("scalac") & "(1)", "scalac.html") & ", " & Link(
      Bold("scalap") & "(1)",
      "scalap.html"
    )
  )

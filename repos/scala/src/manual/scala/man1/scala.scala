...
object scala extends Command {
    ...
    DefinitionList(
      ...
      Definition(
        ...
        "How to execute " & Argument("torun") & ", if it is present. " & "Options for " & Argument(
          "how"
        ) & " are " & Mono("guess") & " (the default), " & Mono("script") & ", " & Mono(
          "jar"
        ) & ", and " & Mono("object") & "."
      ),
      Definition(
  ...
  val description =
    Section(
      ...
      "In all three cases, arbitrary scalac options may be specified. " & "The most common option is to specify a classpath with " & Mono(
        "-classpath"
      ) & ", but see the " & Link(Bold("scalac") & "(1)", "scalac.html") & " page for " & "full details.   ",
      ...
      "If " & Mono("scala") & " is run from an sbaz(1) directory, " & "then it will add to its classpath any jars installed in the " & "lib directory of the sbaz directory.  Additionally, if no " & "-classpath option is specified, then " & Mono(
        "scala"
      ) & " will add " & Quote(".") & ", the current directory, to the " & "end of the classpath."
    )
  ...
  val options = Section(
    ...
    "If any compiler options are specified, they must be first in the " & "command line and must be followed by a bare hypen (" & Quote(
      ...
      "-object"
    ) & " or " & Mono("-script") & " flag, but usually the " & "program can guess correctly."
  )

...
object scala extends Command {
  ...
  val parameters = Section(
    ...
    DefinitionList(
      ...
      Definition(
        ...
        "How to execute " & Argument(
          "torun"
        ) & ", if it is present. " & "Options for " & Argument(
          "how"
        ) & " are " & Mono("guess") & " (the default), " & Mono(
          "script"
        ) & ", " & Mono("jar") & ", and " & Mono("object") & "."
      ),
      Definition(
      ...
      Definition(
        ...
        "Save this compiled version of scripts in order to speed up " & "later executions of the same script.  When running a script, " & "save the compiled version in a file with the same name as the " & "script but with an extension of " & Mono(
          ".jar"
        ) & ".  On subsequent " & "runs of the same script, the pre-compiled " & Mono(
          ".jar"
        ) & " file " & "will be used if it is newer than the script file."
      ),
      Definition(
  ...
  val description =
    Section(
      ...
      NumberedList(
        ...
        "With " & Mono(
          "-howtorun:object"
        ) & " specified, the fully " & "qualified name of a top-level " & "Scala object may be specified.  The object should previously have " & "been compiled using " & Link(
          Bold("scalac") & "(1)",
          "scalac.html"
        ) & ".",
        "With " & Mono(
      ...
      "The method must return a " & Bold(
        "Unit"
      ) & " value, and it must " & "accept a " & Bold(
        "String"
      ) & " array as a parameter.  All arguments " & "specified on the command line will be passed as " & "arguments to the " & Bold(
        "main"
      ) & " method.",
      ...
      "When running a script or using " & Mono(
        "-e"
      ) & ", an already running " & "compilation daemon (fsc) is used, or a new one started on demand.  The " & Mono(
        "-nocompdaemon"
      ) & " or " & Mono("-nc") & " option can be used to " & "prevent this.",
      "If " & Mono(
        "scala"
      ) & " is run from an sbaz(1) directory, " & "then it will add to its classpath any jars installed in the " & "lib directory of the sbaz directory.  Additionally, if no " & "-classpath option is specified, then " & Mono(
        "scala"
      ) & " will add " & Quote(
        "."
      ) & ", the current directory, to the " & "end of the classpath."
    )
    ...
    DefinitionList(
      ...
      Definition(
        ...
        "Specify JDK/JRE home directory. This directory is used to locate " & "the " & MBold(
          "java"
        ) & " command unless " & MBold("JAVACMD") & " variable set."
      ),
      Definition(
        MBold("JAVA_OPTS"),
        SeqPara(
  ...
  val examples = Section(
    ...
    DefinitionList(
      ...
      Definition(
        "Execute a Scala program using a user-defined " & MBold(
          "java"
        ) & " " & "command",
  ...
  val exitStatus = Section(
    ...
    "The " & MBold(
      command
    ) & " command " & "returns a zero exit status if it succeeds. " & "Non zero is returned in case of any error.  If a script or " & "top-level object is executed and returns a value, then that " & "return value is passed on to " & MBold(
      command
    ) & "."
  )
  ...
  val seeAlso = Section(
    ...
    Link(Bold("fsc") & "(1)", "fsc.html") & ", " & Link(
      Bold("scalac") & "(1)",
      "scalac.html"
    ) & ", " & Link(Bold("scaladoc") & "(1)", "scaladoc.html") & ", " & Link(
      Bold("scalap") & "(1)",
      "scalap.html"
    )
  )

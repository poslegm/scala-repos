...
trait ReificationSupport { self: SymbolTable =>
  ...
  class ReificationSupportImpl extends ReificationSupportApi {
    ...
    protected object UnCtor {
      def unapply(
          tree: Tree
      ): Option[(Modifiers, List[List[ValDef]], List[Tree])] =
        tree match {
          case DefDef(
              mods,
    ...
    object SyntacticTraitDef extends SyntacticTraitDefExtractor {
      ...
      def unapply(tree: Tree): Option[
        ...
        case ClassDef(
            ...
            name,
            tparams,
            UnMkTemplate(parents, selfType, ctorMods, vparamss, earlyDefs, body)
            ) if mods.isTrait =>
    ...
    object SyntacticNew extends SyntacticNewExtractor {
      ...
      def unapply(
          tree: Tree
      ): Option[(List[Tree], List[Tree], ValDef, List[Tree])] =
        tree match {
    ...
    object implodePatDefs extends Transformer {
      ...
      def transformStats(trees: List[Tree]): List[Tree] = trees match {
        ...
        case ValDef(
              ...
              Match(
                MaybeTyped(MaybeUnchecked(value), tpt),
                CaseDef(pat, EmptyTree, SyntacticTuple(ids)) :: Nil
              )
            ) :: tail if mods.hasFlag(SYNTHETIC) && mods.hasFlag(ARTIFACT) =>
          ids match {
            case Nil =>
              ValDef(
                NoMods,
            ...
            case _ =>
              ...
              ValDef(
                ...
                Typed(pat, tpt),
                transform(value)
              ) :: transformStats(tail.drop(ids.length))
          }
        case other :: tail =>
    ...
    protected object UnVisitor {
      def unapply(tree: Tree): Option[(TermName, List[CaseDef])] = tree match {
        case Function(
            ...
            Match(MaybeUnchecked(Ident(x2)), cases)
            ) if x1 == x2 =>
          Some((x1, cases))
        case _ => None
      }
    ...
    object SyntacticImport extends SyntacticImportExtractor {
      ...
      private object RenameSelector {
        ...
        def unapply(
            sel: ImportSelector
        ): Option[(TermName, Int, TermName, Int)] =
          sel match {
      ...
      private object RenameSelectorRepr {
        ...
        def unapply(
            tree: Tree
        ): Option[(TermName, Position, TermName, Position)] =
          tree match {
            case Arrow(

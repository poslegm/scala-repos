  ...
  abstract class DefinitionsClass
    ...
    class VarArityClass(
      ...
      val seq: IndexedSeq[ClassSymbol] = (init ++: countFrom
        .to(maxArity)
        .map { i => getRequiredClass("scala." + name + i) }).toVector
    ...
    private def typeArgOfBaseTypeOr(tp: Type, baseClass: Symbol)(
        or: => Type
    ): Type = (tp baseType baseClass).typeArgs match {
      case x :: Nil =>
        val x1 = x
    ...
    def getLanguageFeature(
        name: String,
        owner: Symbol = languageFeatureModule
    ): Symbol =
    ...
    private def specialPolyClass(name: TypeName, flags: Long)(
      ...
      clazz setInfo GenPolyType(
        List(tparam),
        ClassInfoType(parents, newScope, clazz)
      ) markAllCompleted
    }
    ...
    final class RunDefinitions {
      ...
      private lazy val PolySigMethods: Set[Symbol] =
        Set[Symbol](
          ...
          MethodHandle.info.decl(sn.InvokeExact)
        ).filter(_.exists)

...
with ContextTrees with RichCompilationUnits with Picklers {
  ...
  private[interactive] def filesDeleted(
    ...
    val deletedSyms =
      currentTopLevelSyms filter { sym => deletedFiles contains sym.sourceFile }
    for (d <- deletedSyms) {
      d.owner.info.decls unlink d
      deletedTopLevelSyms += d
      currentTopLevelSyms -= d
  ...
  sealed abstract class CompletionResult {
    ...
    def matchingResults(
        nameMatcher: (Name) => Name => Boolean = entered =>
          candidate => candidate.startsWith(entered)
    ): List[M] = {

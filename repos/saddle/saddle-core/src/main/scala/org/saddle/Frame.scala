...
class Frame[RX: ST: ORD, CX: ST: ORD, T: ST](
  ...
  def joinS(
    ...
    val lft =
      indexer.lTake.map { loc => values.map(_.take(loc)) } getOrElse values
    val rgt =
      indexer.rTake.map { loc => other.values.take(loc) } getOrElse other.values
  ...
  def rconcat[U, V](
      other: Frame[RX, CX, U],
      how: JoinType = OuterJoin
  )(implicit wd1: Promoter[T, U, V], mu: ST[U], md: ST[V]): Frame[RX, CX, V] =

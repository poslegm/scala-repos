...
sealed abstract class Xor[+A, +B] extends Product with Serializable {
  ...
  def recoverWith[AA >: A, BB >: B](
      pf: PartialFunction[A, AA Xor BB]
  ): AA Xor BB =
    this match {
  ...
  def withValidated[AA, BB](
      f: Validated[A, B] => Validated[AA, BB]
  ): AA Xor BB =
    f(toValidated).toXor
  ...
  def compare[AA >: A, BB >: B](
      that: AA Xor BB
  )(implicit AA: Order[AA], BB: Order[BB]): Int =
    fold(
  ...
  final def combine[AA >: A, BB >: B](
      that: AA Xor BB
  )(implicit AA: Semigroup[AA], BB: Semigroup[BB]): AA Xor BB =
    this match {
      case Xor.Left(a1) =>
        that match {
...
private[data] sealed abstract class XorInstances extends XorInstances1 {
  ...
  implicit def xorBifunctor: Bitraverse[Xor] =
    new Bitraverse[Xor] {
      def bitraverse[G[_], A, B, C, D](
          fab: Xor[A, B]
      )(f: A => G[C], g: B => G[D])(implicit G: Applicative[G]): G[Xor[C, D]] =
        fab match {
  ...
  implicit def xorInstances[A]
    ...
    new Traverse[A Xor ?] with MonadError[Xor[A, ?], A] {
      ...
      override def recover[B](
          fab: A Xor B
      )(pf: PartialFunction[A, B]): A Xor B =
        fab recover pf
      override def recoverWith[B](
          fab: A Xor B

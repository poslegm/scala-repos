...
object DenseVector
  ...
  def ones[@spec(Double, Int, Float, Long) V: ClassTag: Semiring](
      size: Int
  ): DenseVector[V] = fill[V](size, implicitly[Semiring[V]].one)
  ...
  implicit val canAddD
      : OpAdd.Impl2[DenseVector[Double], DenseVector[Double], DenseVector[
        Double
      ]] = {
  ...
  implicit val canSubD
      : OpSub.Impl2[DenseVector[Double], DenseVector[Double], DenseVector[
        Double
      ]] = {

...
trait DenseVectorOps extends DenseVector_GenericOps {
  ...
  implicit def dv_v_InPlaceOp[
      ...
      implicit @expand.sequence[Op](_ * _, _ / _, (a, b) => b, _ % _, _ pow _) op: Op.Impl2[
        T,
        T,
        T
      ]
  ...
  implicit def dvSubIntoSField[T](
      implicit field: Ring[T],
      ct: ClassTag[T]
  ): OpSub.InPlaceImpl2[DenseVector[T], T] =
  ...
  implicit def dvDivIntoSField[T](
      implicit field: Field[T],
      ct: ClassTag[T]
  ): OpDiv.InPlaceImpl2[DenseVector[T], T] =
...
trait DenseVector_SpecialOps extends DenseVectorOps {
  ...
  implicit val canAddF
      : OpAdd.Impl2[DenseVector[Float], DenseVector[Float], DenseVector[
        Float
      ]] = {
  ...
  implicit val canSubF
      : OpSub.Impl2[DenseVector[Float], DenseVector[Float], DenseVector[
        Float
      ]] = {
  ...
  implicit val canDot_DV_DV_Float: breeze.linalg.operators.OpMulInner.Impl2[
    ...
    new breeze.linalg.operators.OpMulInner.Impl2[DenseVector[Float], DenseVector[
      Float
    ], Float] {
      def apply(a: DenseVector[Float], b: DenseVector[Float]) = {

...
sealed abstract class NonEmptyListInstances extends NonEmptyListInstances0 {
  implicit val nonEmptyList: Traverse1[NonEmptyList]
    ...
    new Traverse1[NonEmptyList]
      ...
      def traverse1Impl[G[_]: Apply, A, B](
          fa: NonEmptyList[A]
      ...
      override def foldMap1[A, B](
          fa: NonEmptyList[A]
      )(f: A => B)(implicit F: Semigroup[B]): B =
      ...
      def cobind[A, B](fa: NonEmptyList[A])(
          f: NonEmptyList[A] => B
      ): NonEmptyList[B] = map(cojoin(fa))(f)

      override def cojoin[A](
          a: NonEmptyList[A]
      ): NonEmptyList[NonEmptyList[A]] = a.tails

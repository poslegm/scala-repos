...
final case class TreeLoc[A](
  ...
  def findChild(p: Tree[A] => Boolean): Option[TreeLoc[A]] = {
    ...
    def split(
        acc: TreeForest[A],
        xs: TreeForest[A]
    ): Option[(TreeForest[A], Tree[A], TreeForest[A])] =
      (acc, xs) match {
  ...
  def cojoin: TreeLoc[TreeLoc[A]] = {
    ...
    def uf[A](
        a: TreeLoc[A],
        f: TreeLoc[A] => Option[TreeLoc[A]]
    ): Stream[Tree[TreeLoc[A]]] = {
      ...
      override def foldLeft[A, B](fa: TreeLoc[A], z: B)(f: (B, A) => B) =
        ParentsT.foldLeft(
          ...
          ForestT.foldLeft(
            fa.rights,
            ForestT
              .foldLeft(fa.lefts, Foldable[Tree].foldLeft(fa.tree, z)(f))(f)
          )(f)
        )(f)
      ...
      override def foldMapLeft1[A, B](
        ...
        ParentsT.foldLeft(
          ...
          ForestT.foldLeft(
            ...
            ForestT.foldLeft(
              fa.lefts,
              Foldable1[Tree].foldMapLeft1(fa.tree)(z)(f)
            )(f)
          )(f)
        )(f)

      override def traverse1Impl[G[_], A, B](
          fa: TreeLoc[A]
      )(f: A => G[B])(implicit G: Apply[G]) = fa.lefts match {
        case l #:: ls =>

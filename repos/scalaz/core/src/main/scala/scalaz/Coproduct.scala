...
private trait CoproductFoldable1[F[_], G[_]]
  ...
  override final def foldMap1[A, B: Semigroup](
      fa: Coproduct[F, G, A]
  )(f: A => B): B =
    fa.foldMap1(f)
...
private trait CoproductTraverse1[F[_], G[_]]
  ...
  override final def traverse1Impl[X[_]: Apply, A, B](
      fa: Coproduct[F, G, A]
  )(f: A => X[B]): X[Coproduct[F, G, B]] =
    fa traverse1 f

...
sealed abstract class Finger[V, A] {
  ...
  private[scalaz] def split1(
      pred: V => Boolean,
      accV: V
  ): (Option[Finger[V, A]], A, Option[Finger[V, A]])
}
...
sealed abstract class FingerTree[V, A](implicit measurer: Reducer[A, V]) {
  ...
  private def split1(
      pred: V => Boolean,
      accV: V
  ): (FingerTree[V, A], A, FingerTree[V, A]) = fold(
  ...
  /**
    ...
    */
  def traverseTree[F[_], V2, B](
      f: A => F[B]
  )(implicit ms: Reducer[B, V2], F: Applicative[F]): F[FingerTree[V2, B]] = {
    def mkDeep(pr: Finger[V2, B])(m: FingerTree[V2, Node[V2, B]])(
        sf: Finger[V2, B]
    ): FingerTree[V2, B] = deep(pr, m, sf)
    fold(
      ...
      (v, pr, m, sf) => {
        ...
        val fmap2 = F.apply2(
          traverseFinger(pr)(f),
          m.traverseTree(n => traverseNode(n)(f))
        )((a, b) => mkDeep(a)(b) _)
  ...
  private def traverseNode[F[_], V2, B](node: Node[V, A])(
      f: A => F[B]
  )(implicit ms: Reducer[B, V2], F: Applicative[F]): F[Node[V2, B]] = {
  ...
  private def traverseFinger[F[_], A, B, V2](digit: Finger[V, A])(
      f: A => F[B]
  )(implicit ms: Reducer[B, V2], F: Applicative[F]): F[Finger[V2, B]] = {
...
sealed abstract class FingerTreeInstances {
  ...
  implicit def fingerMeasure[A, V](
      implicit m: Reducer[A, V]
  ): Reducer[Finger[V, A], V] = {

...
object Cofree extends CofreeInstances {
  ...
  def unfoldC[F[_], A](
      a: A
  )(f: A => F[A])(implicit F: Functor[F]): Cofree[F, A] =
  ...
  def mapUnfold[F[_], W[_], A](
      z: W[A]
  )(f: W ~> F)(implicit W: Comonad[W]): Cofree[F, A] =
...
private trait CofreeFoldable[F[_]] extends Foldable1[Cofree[F, ?]] {
  ...
  override final def foldMap[A, B](
      fa: Cofree[F, A]
...
private trait CofreeFoldable1[F[_]]
  ...
  override final def foldMap1[A, B](
      fa: Cofree[F, A]
  )(f: A => B)(implicit S: Semigroup[B]): B =

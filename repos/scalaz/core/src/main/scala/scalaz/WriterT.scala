...
final case class WriterT[F[_], W, A](run: F[(W, A)]) { self =>
  ...
  def ap[B](
      f: => WriterT[F, W, A => B]
  )(implicit F: Apply[F], W: Semigroup[W]): WriterT[F, W, B] =
    writerT {
      F.apply2(f.run, run) {
...
trait WriterTFunctions {
  ...
  def put[F[_], W, A](
      value: F[A]
  )(w: W)(implicit F: Functor[F]): WriterT[F, W, A] =
  ...
  def putWith[F[_], W, A](
      value: F[A]
  )(w: A => W)(implicit F: Functor[F]): WriterT[F, W, A] =

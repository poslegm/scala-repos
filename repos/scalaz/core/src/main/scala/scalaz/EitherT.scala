...
final case class EitherT[F[_], A, B](run: F[A \/ B]) {
  ...
  def ensure(
      onLeft: => A
  )(f: B => Boolean)(implicit F: Functor[F]): EitherT[F, A, B] =
  ...
  def validationed[AA, BB](
      k: Validation[A, B] => Validation[AA, BB]
  )(implicit F: Functor[F]): EitherT[F, AA, BB] =

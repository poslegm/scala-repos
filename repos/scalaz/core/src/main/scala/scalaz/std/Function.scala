...
sealed trait FunctionInstances0 extends FunctionInstances1 {
  ...
  implicit def function1CovariantByName[T]: Monad[(=> T) => ?]
    ...
    new Monad[(=> T) => ?]
      ...
      def distributeImpl[G[_]: Functor, A, B](
          fa: G[A]
      )(f: A => (=> T) => B): (=> T) => G[B] =
...
trait FunctionInstances extends FunctionInstances0 {
  implicit val function0Instance = new Traverse[Function0]
    ...
    def distributeImpl[G[_], A, B](
        fa: G[A]
    )(f: A => () => B)(implicit G: Functor[G]): () => G[B] =
      () => G.map(fa)(a => f(a)())

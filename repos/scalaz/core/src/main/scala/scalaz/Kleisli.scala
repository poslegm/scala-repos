...
final case class Kleisli[M[_], A, B](run: A => M[B]) { self =>
  ...
  def unlift[N[_], FF[_]](
      ...
      ev: this.type <~< Kleisli[λ[α => N[FF[α]]], A, B]
  ): Kleisli[FF, A, B] =
    kleisli[FF, A, B](a => Comonad[N].copoint(ev(self) run a))

  def unliftId[N[_]](
      implicit M: Comonad[N],
      ev: this.type <~< Kleisli[N[?], A, B]
  ): Reader[A, B] =
    unlift[N, Id]
...
abstract class KleisliInstances extends KleisliInstances0 {
  ...
  implicit def kleisliCatchable[F[_], A](
      implicit F0: Catchable[F]
  ): Catchable[Kleisli[F, A, ?]] =
...
private trait KleisliDistributive[F[_], R]
  ...
  override def distributeImpl[G[_]: Functor, A, B](
      a: G[A]

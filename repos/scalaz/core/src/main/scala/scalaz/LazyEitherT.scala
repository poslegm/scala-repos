...
object LazyEitherT extends LazyEitherTInstances {
  ...
  def lazyEitherTLeftProjectionEIso2[F[_], E]
    ...
    new IsoFunctorTemplate[
      ...
      def to[A](
          fa: LazyEitherT.LeftProjectionT[F, E, A]
      ): LazyEitherT[F, E, A] =
        fa.lazyEitherT
      def from[A](
          ga: LazyEitherT[F, E, A]
      ): LazyEitherT.LeftProjectionT[F, E, A] =
        ga.left
    }
  ...
  def lazyEitherTLeftProjectionIso2[F[_]]
    ...
    new IsoBifunctorTemplate[
      ...
      def to[A, B](
          fa: LazyEitherT.LeftProjectionT[F, A, B]
      ): LazyEitherT[F, A, B] =
        fa.lazyEitherT
      def from[A, B](
          ga: LazyEitherT[F, A, B]
      ): LazyEitherT.LeftProjectionT[F, A, B] =
        ga.left
    }
...
sealed abstract class LazyEitherTInstances1 {
  ...
  implicit def lazyEitherTLeftProjectionFunctor[F[_], L](implicit
      F0: Functor[F]
  ): Functor[LazyEitherT.LeftProjectionT[F, L, ?]] =
...
sealed abstract class LazyEitherTInstances0 extends LazyEitherTInstances1 {
  ...
  implicit def lazyEitherTLeftProjectionMonad[F[_], L](implicit
      F0: Monad[F]
  ): Monad[LazyEitherT.LeftProjectionT[F, L, ?]] =
...
private trait LazyEitherTFunctor[F[_], E]
  ...
  override final def map[A, B](fa: LazyEitherT[F, E, A])(
      f: A => B
  ): LazyEitherT[F, E, B] = fa map (a => f(a))
}
...
private trait LazyEitherTTraverse[F[_], E]
  ...
  def traverseImpl[G[_]: Applicative, A, B](fa: LazyEitherT[F, E, A])(
      f: A => G[B]
  ): G[LazyEitherT[F, E, B]] = fa traverse f
}
...
private trait LazyEitherTBifoldable[F[_]]
  ...
  override final def bifoldMap[A, B, M: Monoid](
      fab: LazyEitherT[F, A, B]
  )(f: A => M)(g: B => M) =

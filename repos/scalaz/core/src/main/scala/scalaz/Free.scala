...
sealed abstract class Free[S[_], A] {
  ...
  final def foldRun[B](b: B)(
      f: (B, S[Free[S, A]]) => (B, Free[S, A])
...
private sealed trait FreeTraverse[F[_]]
  ...
  override final def traverseImpl[G[_], A, B](
      fa: Free[F, A]
  )(f: A => G[B])(implicit G: Applicative[G]): G[Free[F, B]] =
    fa.resume match {
      case -\/(s) =>

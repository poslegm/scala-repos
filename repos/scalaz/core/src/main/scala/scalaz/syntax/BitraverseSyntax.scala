...
final class BitraverseOps[F[_, _], A, B] private[syntax] (val self: F[A, B])(
  ...
  final def bitraverseU[GC, GD](f: A => GC, g: B => GD)(
      implicit G1: UnapplyProduct[Applicative, GC, GD]
  ): G1.M[F[G1.A, G1.B]] =
  ...
  final def bisequenceU[GC, GD](
      implicit eva: A === GC,
      evb: B === GD,
      G1: UnapplyProduct[Applicative, GC, GD]
  ): G1.M[F[G1.A, G1.B]] =
    bitraverseU(eva, evb)
  ////
}
...
sealed trait ToBitraverseOps0 {
  implicit def ToBitraverseOpsUnapply[FA](
      v: FA
...
trait ToBitraverseOps
  ...
  implicit def ToBitraverseOps[F[_, _], A, B](
      v: F[A, B]

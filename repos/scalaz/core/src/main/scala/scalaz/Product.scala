...
private trait ProductFoldable1L[F[_], G[_]]
  ...
  override def foldMap1[A, B](
      fa: (F[A], G[A])
  )(f: A => B)(implicit S: Semigroup[B]): B = {
...
private trait ProductTraverse[F[_], G[_]]
  ...
  def traverseImpl[X[_]: Applicative, A, B](
      a: (F[A], G[A])
  )(f: A => X[B]): X[(F[B], G[B])] =
...
private trait ProductTraverse1L[F[_], G[_]]
  ...
  def traverse1Impl[X[_], A, B](
      a: (F[A], G[A])
  )(f: A => X[B])(implicit X0: Apply[X]): X[(F[B], G[B])] = {
  ...
  override def traverseImpl[X[_]: Applicative, A, B](
      a: (F[A], G[A])
  )(f: A => X[B]): X[(F[B], G[B])] =
...
private trait ProductTraverse1[F[_], G[_]]
  ...
  def traverse1Impl[X[_]: Apply, A, B](
      a: (F[A], G[A])
  )(f: A => X[B]): X[(F[B], G[B])] =
...
private trait ProductDistributive[F[_], G[_]]
  ...
  def distributeImpl[X[_]: Functor, A, B](
      a: X[A]
...
private trait ProductBitraverse[F[_, _], G[_, _]]
  ...
  def bitraverseImpl[X[_]: Applicative, A, B, C, D](
      x: (F[A, B], G[A, B])

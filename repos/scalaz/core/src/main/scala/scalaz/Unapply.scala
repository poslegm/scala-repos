...
sealed abstract class Unapply_5 {
  ...
  implicit def unapplyMFABCDE5[TC[_[_]], F[_], M0[F[_], _, _, _, _, _], A0, B0, C0, D0, E0](
...
sealed abstract class Unapply_3 extends Unapply_4 {
  ...
  implicit def unapplyMFABC3[TC[_[_]], F[_], M0[F[_], _, _, _], A0, B0, C0](
      implicit TC0: TC[M0[F, A0, B0, ?]]
  ): Unapply[TC, M0[F, A0, B0, C0]] {
...
sealed abstract class Unapply_2 extends Unapply_3 {
  ...
  implicit def unapplyMFAB1[TC[_[_]], F[_], M0[F[_], _, _], A0, B0](
      implicit TC0: TC[M0[F, ?, B0]]
  ): Unapply[TC, M0[F, A0, B0]] {
    type M[X] = M0[F, X, B0]
    type A    = A0
  } =
...
sealed abstract class Unapply_1 extends Unapply_2 {
  ...
  implicit def unapplyMABCDEFG7[TC[_[_]], M0[_, _, _, _, _, _, _], A0, B0, C0, D0, E0, F0, G0](
  ...
  implicit def unapplyMABCDEF6[TC[_[_]], M0[_, _, _, _, _, _], A0, B0, C0, D0, E0, F0](
  ...
  implicit def unapplyMABCD4[TC[_[_]], M0[_, _, _, _], A0, B0, C0, D0](
      implicit TC0: TC[M0[A0, B0, C0, ?]]
  ): Unapply[TC, M0[A0, B0, C0, D0]] {
...
sealed abstract class Unapply2_0 {
  ...
  implicit def unapplyMFAB[TC[_[_, _]], F[_], M0[F[_], _, _], A0, B0](
      implicit TC0: TC[M0[F, ?, ?]]
  ): Unapply2[TC, M0[F, A0, B0]] {
    type M[X, Y] = M0[F, X, Y]
    type A       = A0
    type B       = B0
  } =
...
object Unapply21 {
  ...
  implicit def unapply210MFABC[TC[_[_, _], _], F[_, _], M0[_[_], _, _], A0, B0, C](
...
object UnapplyProduct {
  ...
  object SingletonOf {
    implicit def mkSingletonOf[T <: { type A; type M[_] }](
        implicit t: T
    ): SingletonOf[T, t.type] =
      SingletonOf(t)
  }

  implicit def unapply[TC[_[_]], MA0, MB0, U1 <: { type A; type M[_] }, U2 <: { type A; type M[_] }](

...
sealed abstract class PLensFamily[A1, A2, B1, B2] {
  ...
  def compose[C1, C2](
      that: PLensFamily[C1, C2, A1, A2]
  ): PLensFamily[C1, C2, B1, B2] =
    plensFamily(c =>
  ...
  def andThen[C1, C2](
      that: PLensFamily[B1, B2, C1, C2]
  ): PLensFamily[A1, A2, C1, C2] =
    that compose this
  ...
  def product[C1, C2, D1, D2](
      that: PLensFamily[C1, C2, D1, D2]
...
object PLensFamily extends PLensInstances with PLensFunctions {
  ...
  def apply[A1, A2, B1, B2](
      r: A1 => Option[IndexedStore[B1, B2, A2]]
...
trait PLensFamilyFunctions extends PLensInstances {
  ...
  def eitherLensFamily[S1, S2, A, B](
      l: PLensFamily[S1, S2, A \/ B, A \/ B]
  ): (PLensFamily[S1, S2, A, A], PLensFamily[S1, S2, B, B]) =
    (
...
trait PLensFunctions extends PLensInstances with PLensFamilyFunctions {
  ...
  def ephemeralStreamLookupByPLens[K, V](
      p: K => Boolean
  ): EphemeralStream[(K, V)] @?> V = {
    import EphemeralStream.cons

    @annotation.tailrec
    def lookupr(
        t: (EphemeralStream[(K, V)], (K, V), EphemeralStream[(K, V)])
    ...
    plens(s =>
      if (s.isEmpty) None
        ...
        lookupr(
          (EphemeralStream.emptyEphemeralStream, s.head(), s.tail())
        ) map {

...
final class BijectionT[F[_], G[_], A, B] private[scalaz] (
  ...
  /** alias for `compose` */
  def <=<[C](
      that: BijectionT[F, G, C, A]
  )(implicit FM: Bind[F], GM: Bind[G]): BijectionT[F, G, C, B] =
    compose(that)

  def andThen[C](
      that: BijectionT[F, G, B, C]
...
object BijectionT extends BijectionTInstances {
  ...
  def liftBijection[F[_], G[_], A, B](
      t: A => B,
      f: B => A
  )(implicit PF: Applicative[F], PG: Applicative[G]): BijectionT[F, G, A, B] =
...
private trait BijectionTSplit[F[_], G[_]]
  ...
  def split[A, B, C, D](
      ab: BijectionT[F, G, A, B],
      cd: BijectionT[F, G, C, D]
  ): BijectionT[F, G, (A, C), (B, D)] =

...
trait Bitraverse[F[_, _]] extends Bifunctor[F] with Bifoldable[F] { self =>
  ...
  def traverseSTrampoline[S, G[_]: Applicative, A, B, C, D](
      fa: F[A, B]
    ...
    Kleisli[G, S, F[C, D]](s => {
      val kl =
        bitraverse[λ[α => Kleisli[Trampoline, S, G[α]]], A, B, C, D](fa)(z =>
          Kleisli[Id, S, G[C]](i => f(z)(i)).lift[Trampoline]
        )(z => Kleisli[Id, S, G[D]](i => g(z)(i)).lift[Trampoline])
      kl.run(s).run
    })
  }
  ...
  override def bifoldLeft[A, B, C](fa: F[A, B], z: C)(
      f: (C, A) => C
  )(g: (C, B) => C): C =
  ...
  def embedLeft[G[_]](
      implicit G0: Traverse[G]
  ): Bitraverse[λ[(α, β) => F[G[α], β]]] =
    embed[G, Id.Id]

  /** Embed a Traverse on the right side of this Bitraverse . */
  def embedRight[H[_]](
      implicit H0: Traverse[H]
  ): Bitraverse[λ[(α, β) => F[α, H[β]]]] =
    embed[Id.Id, H]

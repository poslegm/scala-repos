...
class ScaloidCodeGenerator(
  ...
  def richClassDef =
    s"""$richClassScalaDoc
       |${deprecated}class Rich${cls.name}[This <: ${genType(
         cls.tpe,
         erased = true
       )}](val basis: This) extends $helperTraitName[This]
       |
       |$helperTraitScalaDoc
  ...
  class ConstructorGenerator(con: ScalaConstructor) {
    ...
    private def constTypeParams = {
      val argStrings =
        con.paramedTypes.map(
          paramedType(_, define = true)
  ...
  def noGetter(name: String) =
    s"""@inline def ${safeIdent(
      name
    )}(implicit no: NoGetterForThisProperty): Nothing = throw new Error("Android does not support the getter for '${name}'")"""

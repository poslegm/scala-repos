...
class BasicDirectivesExamplesSpec extends RoutingSpec {
  ...
  "textract" in {
    ...
    Get("/abcdef?ghi=12") ~> route ~> check {
      responseAs[
        String
      ] shouldEqual "The path is /abcdef and the query is ghi=12"
    }
    //#
  }
  "tprovide" in {
  ...
  "extractRequest-example" in {
    ...
    Get("/") ~> route ~> check {
      responseAs[
        String
      ] shouldEqual "Request method is GET and content-type is none/none"
    }
    //#
  }
  "extractSettings-examples" in {
    ...
    Get("/") ~> route ~> check {
      responseAs[
        String
      ] shouldEqual s"RoutingSettings.renderVanityFooter = true"
    }
    //#
  }
  "mapSettings-examples" in {
    ...
    Get("/") ~> route ~> check {
      responseAs[
        String
      ] shouldEqual s"RoutingSettings.fileGetConditional = false"
    }
    //#
  }

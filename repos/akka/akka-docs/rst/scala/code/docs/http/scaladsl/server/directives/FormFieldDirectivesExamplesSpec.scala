...
class FormFieldDirectivesExamplesSpec extends RoutingSpec {
  "formFields" in {
    ...
    Post("/", FormData("color" -> "blue", "age" -> "68")) ~> route ~> check {
      responseAs[
        String
      ] shouldEqual "The color is 'blue' and the age ten years ago was 58"
    }
  ...
  "formFieldMap" in {
    ...
    Post("/", FormData("color" -> "blue", "count" -> "42")) ~> route ~> check {
      responseAs[
        String
      ] shouldEqual "The form fields are color = 'blue', count = '42'"
    }
  ...
  "formFieldMultiMap" in {
    ...
    Post("/", FormData("color" -> "blue", "count" -> "42")) ~> route ~> check {
      responseAs[
        String
      ] shouldEqual "There are form fields color -> 1, count -> 1"
    }
    Post(
      "/",
      FormData("x" -> "23", "x" -> "4", "x" -> "89")
    ) ~> route ~> check {
  ...
  "formFieldSeq" in {
    ...
    Post(
      ...
      responseAs[
        String
      ] shouldEqual "The form fields are x = '23', x = '4', x = '89'"
    }
  }
}

...
class HttpHeaderSpec extends FreeSpec with Matchers {
  ...
  "The HTTP header model must correctly parse and render the headers" - {
    ...
    "User-Agent" in {
      ...
      "User-Agent: foo(bar)(baz)" =!= `User-Agent`(
        ...
        ProductVersion(comment = "baz")
      ).renderedTo("foo (bar) (baz)")
    }
  ...
  "The header parser should" - {
    ...
    "allow UTF8 characters in RawHeaders" in {
      parse(
        "Flood-Resistant-Hammerdrill",
        "árvíztűrő ütvefúrógép"
      ) shouldEqual ParsingResult
        .Ok(
    ...
    "parse with custom uri parsing mode" in {
      ...
      HeaderParser
        .parseFull(
          ...
          HeaderParser.Settings(uriParsingMode = Uri.ParsingMode.Relaxed)
        ) shouldEqual Right(Location(targetUri))
    }
  }
  ...
  def renderFromHeaderTo(
      header: TestHeader,
      line: String
  ): Matcher[HttpHeader.ParsingResult] =
    Matcher {

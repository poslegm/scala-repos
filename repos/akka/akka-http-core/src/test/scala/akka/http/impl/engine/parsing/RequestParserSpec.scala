...
class RequestParserSpec extends FreeSpec with Matchers with BeforeAndAfterAll {
  ...
  private class Test {
    ...
    def strictEqualify[T](
        x: Either[T, HttpRequest]
    ): Either[T, StrictEqualHttpRequest] =
    ...
    def generalMultiParseTo(
        expected: Either[RequestOutput, HttpRequest]*
    ): Matcher[Seq[String]] =
    ...
    def generalRawMultiParseTo(
      ...
      equal(expected.map(strictEqualify)).matcher[Seq[
        Either[RequestOutput, StrictEqualHttpRequest]
      ]] compose multiParse(parser)

    def multiParse(
        parser: HttpRequestParser
    )(input: Seq[String]): Seq[Either[RequestOutput, StrictEqualHttpRequest]] =
      Source(input.toList)
    ...
    private def compactEntityChunks(
        data: Source[ChunkStreamPart, Any]
    ): Future[Seq[ChunkStreamPart]] =

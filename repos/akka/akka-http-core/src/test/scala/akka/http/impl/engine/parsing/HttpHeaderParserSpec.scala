...
class HttpHeaderParserSpec
  ...
  "The HttpHeaderParser" should {
    ...
    "parse and cache an X-Forwarded-For with a hostname in it as a RawHeader" in new TestSetup() {
      ...
      )() shouldEqual RawHeader("x-forwarded-for", "1.2.3.4, akka.io")
    }
    ...
    "parse and cache a modelled header with line-folding" in new TestSetup() {
      parseAndCache("Connection: foo,\r\n bar\r\nx")(
        "Connection: foo,\r\n bar\r\nx"
      ) shouldEqual Connection("foo", "bar")
    }
    ...
    "parse and cache a header with UTF8 chars in the value" in new TestSetup() {
      ...
      )() shouldEqual RawHeader("2-UTF8-Bytes", "árvíztűrő ütvefúrógép")
      parseAndCache(
        "3-UTF8-Bytes: The € or the $?\r\nx"
      )() shouldEqual RawHeader("3-UTF8-Bytes", "The € or the $?")
      parseAndCache(
        "4-UTF8-Bytes: Surrogate pairs: \uD801\uDC1B\uD801\uDC04\uD801\uDC1B!\r\nx"
      )() shouldEqual RawHeader(
        "4-UTF8-Bytes",
        "Surrogate pairs: \uD801\uDC1B\uD801\uDC04\uD801\uDC1B!"
      )
    }
    ...
    "produce an error message for lines with an illegal header name" in new TestSetup() {
      the[ParsingException] thrownBy parseLine(
        " Connection: close\r\nx"
      ) should have message "Illegal character ' ' in header name"
      the[ParsingException] thrownBy parseLine(
        "Connection : close\r\nx"
      ) should have message "Illegal character ' ' in header name"
      the[ParsingException] thrownBy parseLine(
    ...
    "produce an error message for lines with a too-long header name" in new TestSetup() {
      ...
      ) should have message "HTTP header name exceeds the configured limit of 60 characters"
    }
    ...
    "produce an error message for lines with a too-long header value" in new TestSetup() {
      ...
      ) should have message "HTTP header value exceeds the configured limit of 1000 characters"
    }
  ...
  abstract class TestSetup(primed: Boolean = true) {
    ...
    @tailrec final def nextRandomString(
        charGen: () ⇒ Char,
        len: Int,
        sb: JStringBuilder = new JStringBuilder
    ): String =

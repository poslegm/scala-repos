...
class ResponseRendererSpec
  ...
  "The response preparation logic should properly render" - {
    "a response with no body," - {
      ...
      "a custom Date header" in new TestSetup() {
        HttpResponse(
          200,
          List(Date(DateTime(2011, 8, 26, 10, 11, 59)))
        ) should renderTo {
          """HTTP/1.1 200 OK
      ...
      "status 304 and a few headers" in new TestSetup() {
        HttpResponse(
          304,
          List(RawHeader("X-Fancy", "of course"), Age(0))
        ) should renderTo {
      ...
      "status 400, a few headers and a body with an explicitly suppressed Content Type header" in new TestSetup() {
        HttpResponse(
          ...
          HttpEntity(
            ...
            ByteString("Small f*ck up overhere!")
          )
        ) should renderTo {
          """HTTP/1.1 400 Bad Request
              |Age: 30
      ...
      "status 400 and a few headers" in new TestSetup() {
        HttpResponse(
          ...
          entity = Default(
            ContentTypes.`text/plain(UTF-8)`,
            23,
            source(ByteString("Small f*ck up overhere!"))
          )
        ) should renderTo {
          """HTTP/1.1 400 Bad Request
              |Age: 30
      ...
      "consisting of two parts" in new TestSetup() {
        ResponseRenderingContext(
          HttpResponse(
            ...
            entity = CloseDelimited(
              ContentTypes.`application/json`,
              source(ByteString("abc"), ByteString("defg"))
            )
          )
        ) should renderTo(
          """HTTP/1.1 200 OK
    ...
    "a chunked response" - {
      "with empty entity" in new TestSetup() {
        ...
        HttpResponse(
          200,
          List(Age(30)),
          Chunked(ContentTypes.NoContentType, source())
        ) should renderTo {
          """HTTP/1.1 200 OK
              |Age: 30
              |Server: akka-http/1.0.0
      ...
      "with empty entity but non-default Content-Type" in new TestSetup() {
        ...
        HttpResponse(
          200,
          List(Age(30)),
          Chunked(ContentTypes.`application/json`, source())
        ) should renderTo {
          """HTTP/1.1 200 OK
              |Age: 30
              |Server: akka-http/1.0.0
      ...
      "with one chunk and no explicit LastChunk" in new TestSetup() {
        HttpResponse(entity =
          Chunked(ContentTypes.`text/plain(UTF-8)`, source("Yahoooo"))
        ) should renderTo {
          """HTTP/1.1 200 OK
            |Server: akka-http/1.0.0
            |Date: Thu, 25 Aug 2011 09:10:29 GMT
      ...
      "with a custom Transfer-Encoding header" in new TestSetup() {
        HttpResponse(
          headers =
            List(`Transfer-Encoding`(TransferEncodings.Extension("fancy"))),
          entity = Chunked(ContentTypes.`text/plain(UTF-8)`, source("Yahoooo"))
        ) should renderTo {
          """HTTP/1.1 200 OK
  ...
  class TestSetup(
    ...
    def renderTo(
        expected: String,
        close: Boolean
    ): Matcher[ResponseRenderingContext] =

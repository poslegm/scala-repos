  ...
  "HttpEntity" - {
    "support dataBytes" - {
      ...
      "Chunked w/o LastChunk" in {
        Chunked(
          tpe,
          source(Chunk(abc), Chunk(fgh), Chunk(ijk))
        ) must collectBytesTo(abc, fgh, ijk)
      }
      "Chunked with LastChunk" in {
        Chunked(
          tpe,
          source(Chunk(abc), Chunk(fgh), Chunk(ijk), LastChunk)
        ) must collectBytesTo(abc, fgh, ijk)
      }
    }
    ...
    "support toStrict" - {
      ...
      "Chunked w/o LastChunk" in {
        Chunked(
          tpe,
          source(Chunk(abc), Chunk(fgh), Chunk(ijk))
        ) must strictifyTo(Strict(tpe, abc ++ fgh ++ ijk))
      }
      "Chunked with LastChunk" in {
        Chunked(
          tpe,
    ...
    "support transformDataBytes" - {
      ...
      "Chunked w/o LastChunk" in {
        Chunked(
          tpe,
          source(Chunk(abc), Chunk(fgh), Chunk(ijk))
        ) must transformTo(Strict(tpe, doubleChars("abcfghijk") ++ trailer))
      }
      "Chunked with LastChunk" in {
        Chunked(
          tpe,
      ...
      "Chunked with extra LastChunk" in {
        Chunked(
          tpe,
          source(Chunk(abc), Chunk(fgh), Chunk(ijk), LastChunk, LastChunk)
        ) must transformTo(Strict(tpe, doubleChars("abcfghijk") ++ trailer))
      }
    }
    "support toString" - {

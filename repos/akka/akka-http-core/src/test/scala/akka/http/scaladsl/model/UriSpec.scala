...
class UriSpec extends WordSpec with Matchers {
  ...
  "URIs" should {
    ...
    "be correctly parsed from and rendered to simple test examples" in {
      ...
      // render
      Uri(
        "https://server.com/path/to/here?st=12345"
    ...
    "properly complete a normalization cycle" in {
      ...
      ) shouldEqual "example://a/b/c/%7Bfoo%7D"
      ...
      normalize(
        "example.com:21"
      ) shouldEqual "example.com:21" // example.com is parsed as the SCHEME (which is correct)
      ...
      ) shouldEqual "//user:pass@[::1]:80/segment/index.html?query#frag"
      normalize(
        "http://[::1]:80/segment/index.html?query#frag"
      ) shouldEqual "http://[::1]/segment/index.html?query#frag"
      normalize(
        "http://user:pass@[::1]/segment/index.html?query#frag"
      ) shouldEqual "http://user:pass@[::1]/segment/index.html?query#frag"
      normalize(
        "http://user:pass@[::1]:80?query#frag"
      ) shouldEqual "http://user:pass@[::1]?query#frag"
      normalize(
        "http://user:pass@[::1]/segment/index.html#frag"
      ) shouldEqual "http://user:pass@[::1]/segment/index.html#frag"
      normalize(
        "http://user:pass@[::1]:81/segment/index.html?query"
      ) shouldEqual "http://user:pass@[::1]:81/segment/index.html?query"
      ...
      ) shouldEqual "//user:pass@localhost/one/two/three"
      normalize("http://www.example.com/") shouldEqual "http://www.example.com/"
      normalize(
        "http://sourceforge.net/projects/uriparser/"
      ) shouldEqual "http://sourceforge.net/projects/uriparser/"
      normalize(
        "http://sourceforge.net/project/platformdownload.php?group_id=182840"
      ) shouldEqual "http://sourceforge.net/project/platformdownload.php?group_id=182840"
    ...
    "properly render as HTTP request target origin forms" in {
      Uri(
        "http://example.com/foo/bar?query=1#frag"

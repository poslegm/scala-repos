...
private[parser] trait CommonRules {
  ...
  def comment: Rule0 =
    rule {
      ws('(') ~ clearSB() ~ zeroOrMore(
        ctext | `quoted-cpair` | `nested-comment`
      ) ~ ws(')')
    }
  ...
  def `nested-comment` = {
    ...
    rule {
      &('(') ~ run(saved = sb.toString) ~
        (comment ~ prependSB(saved + " (") ~ appendSB(')') | setSB(
          saved
        ) ~ test(false))
    }
  }
  ...
  def month =
    rule(
      "Jan" ~ push(1) | "Feb" ~ push(2) | "Mar" ~ push(3) | "Apr" ~ push(
        4
      ) | "May" ~ push(5) | "Jun" ~ push(6) | "Jul" ~ push(7) | "Aug" ~ push(
        8
      ) | "Sep" ~ push(9) | "Oct" ~ push(10) | "Nov" ~ push(11) | "Dec" ~ push(
        12
      )
    )
  ...
  def `day-name-l` =
    rule(
      "Sunday" ~ push(0) | "Monday" ~ push(1) | "Tuesday" ~ push(
        2
      ) | "Wednesday" ~ push(3) | "Thursday" ~ push(4) | "Friday" ~ push(
        5
      ) | "Saturday" ~ push(6)
    )
  ...
  def qvalue =
    rule {
      // a bit more lenient than the spec
      capture(
        '0' ~ optional('.' ~ zeroOrMore(DIGIT)) | '.' ~ oneOrMore(
          DIGIT
        ) | '1' ~ optional('.' ~ zeroOrMore('0'))
      ) ~> (_.toFloat) ~ OWS
    }
  ...
  def `optional-cookie-pair`: Rule1[Option[HttpCookiePair]] =
    rule {
      (`cookie-pair` ~ &(`cookie-separator`) ~> (Some(
        _: HttpCookiePair
  ...
  def `extension-av` =
    rule {
      !(ignoreCase("expires=") | ignoreCase("max-age=") | ignoreCase(
        "domain="
      ) | ignoreCase("path=") | ignoreCase("secure") | ignoreCase(
        "httponly"
      )) ~ capture(zeroOrMore(`av-octet`)) ~ OWS ~> {
  ...
  def `transfer-coding` =
    rule(
      ignoreCase("chunked") ~ OWS ~ push(
        TransferEncodings.chunked
      ) | ignoreCase("gzip") ~ OWS ~ push(TransferEncodings.gzip) | ignoreCase(
        "deflate"
      ) ~ OWS ~ push(TransferEncodings.deflate) | ignoreCase(
        "compress"
      ) ~ OWS ~ push(TransferEncodings.compress) | `transfer-extension`
    )
  ...
  def longNumberCappedAtIntMaxValue =
    rule {
      capture((1 to 11).times(DIGIT)) ~> (s ⇒
        math.min(s.toLong, Int.MaxValue)
      ) ~ zeroOrMore(DIGIT) ~ OWS
    }

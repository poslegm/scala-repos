...
private[parser] trait CommonRules {
  ...
  def month =
    rule(
      "Jan" ~ push(1) | "Feb" ~ push(2) | "Mar" ~ push(3) | "Apr" ~ push(4) | "May" ~ push(
        5
      ) | "Jun" ~ push(6) | "Jul" ~ push(7) | "Aug" ~ push(8) | "Sep" ~ push(9) | "Oct" ~ push(
        10
      ) | "Nov" ~ push(11) | "Dec" ~ push(12)
    )
  ...
  def `day-name-l` =
    rule(
      "Sunday" ~ push(0) | "Monday" ~ push(1) | "Tuesday" ~ push(2) | "Wednesday" ~ push(
        3
      ) | "Thursday" ~ push(4) | "Friday" ~ push(5) | "Saturday" ~ push(6)
    )
  ...
  def `extension-av` = rule {
    !(ignoreCase("expires=") | ignoreCase("max-age=") | ignoreCase("domain=") | ignoreCase(
      "path="
    ) | ignoreCase("secure") | ignoreCase("httponly")) ~ capture(
      zeroOrMore(`av-octet`)
    ) ~ OWS ~> { (c: HttpCookie, s: String) ⇒ c.copy(extension = Some(s)) }
  }
  ...
  def `transfer-coding` =
    rule(
      ignoreCase("chunked") ~ OWS ~ push(TransferEncodings.chunked) | ignoreCase(
        "gzip"
      ) ~ OWS ~ push(TransferEncodings.gzip) | ignoreCase("deflate") ~ OWS ~ push(
        TransferEncodings.deflate
      ) | ignoreCase("compress") ~ OWS ~ push(TransferEncodings.compress) | `transfer-extension`
    )
  ...
  def longNumberCapped =
    rule(
      (capture((1 to 18).times(DIGIT)) ~ !DIGIT ~> (_.toLong) | oneOrMore(DIGIT) ~ push(
        999999999999999999L
      )) ~ OWS
    )

...
private[http] object HttpServerBluePrint {
  def apply(
    ...
    val theStack =
      userHandlerGuard(settings.pipeliningLimit) atop requestTimeoutSupport(
        settings.timeouts.requestTimeout
      ) atop requestPreparation(settings) atop controller(
        settings,
        log
      ) atop parsingRendering(settings, log) atop websocketSupport(
        settings,
        log
      ) atop tlsSupport
  ...
  final class PrepareRequests(settings: ServerSettings)
    ...
    override def createLogic(inheritedAttributes: Attributes) =
      new GraphStageLogic(shape) with InHandler with OutHandler {
          ...
          grab(in) match {
            case RequestStart(
              ...
              val entity =
                createEntity(
                  entityCreator
                ) withSizeLimit settings.parserSettings.maxContentLength
              push(
                out,
                HttpRequest(
                  effectiveMethod,
        ...
        def createEntity(
            creator: EntityCreator[RequestOutput, RequestEntity]
        ): RequestEntity =
          creator match {
        ...
        def streamRequestEntity(
            creator: (
                Source[ParserOutput.RequestOutput, NotUsed]
            ) => RequestEntity
        ): RequestEntity = {
  ...
  private class TimeoutAccessImpl(
    ...
    def update(
        timeout: Duration,
        handler: Function[model.HttpRequest, model.HttpResponse]
    ): Unit =

...
class Http(system: ExtendedActorSystem) extends akka.actor.Extension {
  ...
  def serverLayer(
    ...
    adaptServerLayer(
      delegate
        .serverLayer(settings.asScala, remoteAddress.asScala, log)(materializer)
    )
  ...
  private def adaptServerLayer(
      serverLayer: scaladsl.Http.ServerLayer
  ): BidiFlow[
    HttpResponse,
    SslTlsOutbound,
    SslTlsInbound,
  ...
  private def adaptClientLayer(
      clientLayer: scaladsl.Http.ClientLayer
  ): BidiFlow[
    HttpRequest,
    SslTlsOutbound,
    SslTlsInbound,
  ...
  private def adaptWsBidiFlow(
      wsLayer: scaladsl.Http.WebSocketClientLayer
  ): BidiFlow[Message, SslTlsOutbound, SslTlsInbound, Message, CompletionStage[
    WebSocketUpgradeResponse
  ]] =
    new BidiFlow(
  ...
  private def adaptWsFlow[Mat](
      javaFlow: Flow[Message, Message, Mat]

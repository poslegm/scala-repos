  ...
  "Remoting" must {
    ...
    "send warning message for wrong address" in {
      filterEvents(
        ...
        system.actorFor(
          "akka.test://nonexistingsystem@localhost:12346/user/echo"
        ) ! "ping"
      }
    }
    ...
    "send dead letters on remote if actor does not exist" in {
      EventFilter
        ...
        .intercept {
          system.actorFor(
            "akka.test://remote-sys@localhost:12346/does/not/exist"
          ) ! "buh"
        }(remoteSystem)
    }
    ...
    "not send to remote re-created actor with same name" in {
      ...
      system.actorFor(
        "akka.test://remote-sys@localhost:12346/user/otherEcho1"
      ) ! 76
      expectMsg(76)
    ...
    "select actors across node boundaries" in {
      ...
      system.actorSelection(
        child.path / "grandchild" / "grandgrandchild"
      ) ! Identify("idReq10")

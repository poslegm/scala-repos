...
class RangeDirectivesSpec extends RoutingSpec with Inspectors with Inside {
  lazy val wrs =
    mapSettings(
      _.withRangeCountLimit(10).withRangeCoalescingThreshold(1L)
    ) & withRangeSupport
  ...
  "The `withRangeSupport` directive" should {
    ...
    "return a mediaType of 'multipart/byteranges' for a ranged request with multiple ranges" in {
      Get() ~> addHeader(
        Range(ByteRange(0, 10), ByteRange(0, 10))
      ) ~> completeWithRangedBytes(10) ~> check {
        mediaType.withParams(
          Map.empty
        ) shouldEqual MediaTypes.`multipart/byteranges`
      }
    }

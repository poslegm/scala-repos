...
abstract class CoderSpec
  ...
  s"The ${Coder.encoding.value} codec" should {
    ...
    "not throw an error if a subsequent block is corrupt" in {
      ...
      ourDecode(
        Seq(encode("Hello,"), encode(" dear "), corruptContent).join
      ) should readAs("Hello, dear ")
    }
    "decompress in very small chunks" in {
      val compressed = encode("Hello")

      decodeChunks(
        Source(Vector(compressed.take(10), compressed.drop(10)))
      ) should readAs("Hello")
    }
    "support chunked round-trip encoding/decoding" in {
      ...
      val compressedChunks =
        chunks.map { chunk ⇒ comp.compressAndFlush(chunk) } :+ comp.finish()

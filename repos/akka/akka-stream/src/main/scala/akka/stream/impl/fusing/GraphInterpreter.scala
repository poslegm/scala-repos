...
private[akka] object GraphInterpreter {
  ...
  object GraphAssembly {
    ...
    final def apply(
        ...
        stages: GraphStageWithMaterializedValue[Shape, _]*
    ): GraphAssembly = {
...
private[stream] final class GraphInterpreter(
  ...
  def attachUpstreamBoundary(
      connection: Int,
      logic: UpstreamBoundaryStageLogic[_]
  ): Unit = {
  ...
  def attachDownstreamBoundary(
      connection: Int,
      logic: DownstreamBoundaryStageLogic[_]
  ): Unit = {
      ...
      if (Debug)
        println(
          s"$Name PUSH ${outOwnerName(connection)} -> ${inOwnerName(connection)}, ${connectionSlots(
            connection
          )} (${inHandlers(connection)}) [${inLogicName(connection)}]"
        )
    ...
    if ((code & (Pushing | InClosed | OutClosed)) == Pushing) {
      ...
      if (Debug)
        println(s"$Name PULL ${inOwnerName(connection)} -> ${outOwnerName(
          connection
        )} (${outHandlers(connection)}) [${outLogicName(connection)}]")
      ...
      portStates(i) match {
        case InReady ⇒
          builder.append(s"""  ${nameIn(i)} -> ${nameOut(
            i
          )} [label=shouldPull; color=blue]""")
        case OutReady ⇒
          builder.append(s"""  ${nameOut(i)} -> ${nameIn(
            i
          )} [label=shouldPush; color=red];""")
        case x if (x | InClosed | OutClosed) == (InClosed | OutClosed) ⇒
          builder.append(s"""  ${nameIn(i)} -> ${nameOut(
            i
          )} [style=dotted; label=closed dir=both];""")
        case _ ⇒
      }
      builder.append("\n")
    }

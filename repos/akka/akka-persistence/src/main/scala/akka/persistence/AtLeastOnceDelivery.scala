...
trait AtLeastOnceDeliveryLike extends Eventsourced {
  ...
  private val defaultRedeliverInterval: FiniteDuration = Persistence(
    context.system
  ).settings.atLeastOnceDelivery.redeliverInterval
  ...
  private val defaultWarnAfterNumberOfUnconfirmedAttempts: Int = Persistence(
    context.system
  ).settings.atLeastOnceDelivery.warnAfterNumberOfUnconfirmedAttempts
...
abstract class UntypedPersistentActorWithAtLeastOnceDelivery
  ...
  def deliver(
      destination: ActorPath,
      deliveryIdToMessage: akka.japi.Function[java.lang.Long, Object]
  ): Unit =

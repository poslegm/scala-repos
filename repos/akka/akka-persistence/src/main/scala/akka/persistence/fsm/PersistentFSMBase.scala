...
trait PersistentFSMBase[S, D, E]
  ...
  final def onTermination(
      terminationHandler: PartialFunction[StopEvent, Unit]
  ): Unit =
...
abstract class AbstractPersistentFSMBase[S, D, E]
  ...
  final def onTransition(
      transitionHandlerBuilder: FSMTransitionHandlerBuilder[S]
  ): Unit =
    onTransition(

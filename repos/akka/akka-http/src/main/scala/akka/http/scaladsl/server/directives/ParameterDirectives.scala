...
object ParameterDirectives extends ParameterDirectives {
  ...
  object ParamDef {
    ...
    private def extractParameter[A, B](
        f: A ⇒ Directive1[B]
    ): ParamDefAux[A, Directive1[B]] = paramDef(f)
    ...
    implicit def forNOUR[T]
        : ParamDefAux[NameOptionUnmarshallerReceptacle[T], Directive1[
          Option[T]
        ]] =
    ...
    object ConvertParamDefAndConcatenate extends BinaryPolyFunc {
      implicit def from[P, TA, TB](
          ...
          ev: Join[TA, TB]
      ): BinaryPolyFunc.Case[Directive[TA], P, ConvertParamDefAndConcatenate.type] {

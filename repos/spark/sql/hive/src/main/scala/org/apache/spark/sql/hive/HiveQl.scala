...
private[hive] class HiveQl(conf: ParserConf)
      ...
      case Token("TOK_CREATETABLE", children) if children.collect {
        ...
        children.collect {
          ...
          case Token(
                ...
                Token(
                  "TOK_SERDENAME",
                  Token(serdeName, Nil) :: otherProps
                ) :: Nil
              ) =>
            tableDesc =
  ...
  protected override def nodeToTransformation(
      node: ASTNode,
      child: LogicalPlan
  ): Option[logical.ScriptTransformation] =
    node match {
      case Token(
            "TOK_SELEXPR",
            Token(
          ...
          clause match {
            ...
            case Token(
                  ...
                  Token(serdeClass, Nil) :: Token(
                    "TOK_TABLEPROPERTIES",
                    Token("TOK_TABLEPROPLIST", propsClause) :: Nil
                  ) :: Nil
                ) :: Nil =>

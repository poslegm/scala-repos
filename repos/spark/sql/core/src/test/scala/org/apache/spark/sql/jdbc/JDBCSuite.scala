...
class JDBCSuite
  ...
  test("SELECT COUNT(1) WHERE (predicates)") {
    ...
    assert(
      sql(
        "SELECT COUNT(1) FROM foobar WHERE NAME = 'mary'"
      ).collect.toSet === Set(Row(1))
    )
  }
  ...
  test("compile filters") {
    ...
    assert(
      doCompileFilter(
        And(EqualTo("col0", 0), EqualTo("col1", "def"))
      ) === "(col0 = 0) AND (col1 = 'def')"
    )
    assert(
      doCompileFilter(
        Or(EqualTo("col0", 2), EqualTo("col1", "ghi"))
      ) === "(col0 = 2) OR (col1 = 'ghi')"
    )
    ...
    assert(
      doCompileFilter(
        LessThan("col4", Date.valueOf("1983-08-04"))
      ) === "col4 < '1983-08-04'"
    )

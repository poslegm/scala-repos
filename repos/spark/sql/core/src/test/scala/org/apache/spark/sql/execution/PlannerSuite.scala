  ...
  test(
    ...
    val simpleTypes =
      NullType :: BooleanType :: ByteType :: ShortType :: IntegerType :: LongType :: FloatType :: DoubleType :: DecimalType(
        10,
        5
      ) :: DecimalType.SYSTEM_DEFAULT :: DateType :: TimestampType :: StringType :: BinaryType :: Nil
    ...
    val complexTypes =
      ArrayType(DoubleType, true) :: ArrayType(StringType, false) :: MapType(
        ...
        StringType,
        true
      ) :: MapType(IntegerType, ArrayType(DoubleType), false) :: StructType(
        Seq(
  ...
  test("SPARK-11390 explain should print PushedFilters of PhysicalRDD") {
    withTempPath { file =>
      ...
      withTempTable("testPushed") {
        val exp =
          sql(
            "select * from testPushed where key = 15"

...
private[sql] trait SQLTestData { self =>
      ...
      sqlContext.sparkContext
        .parallelize(
          TestData2(1, 1) :: TestData2(1, 2) :: TestData2(2, 1) :: TestData2(
            2,
            2
          ) :: TestData2(3, 1) :: TestData2(3, 2) :: Nil,
          2
        )
        .toDF()
      ...
      sqlContext.sparkContext
        .parallelize(
          LargeAndSmallInts(2147483644, 1) :: LargeAndSmallInts(1, 2) :: LargeAndSmallInts(
            2147483645,
            1
          ) :: LargeAndSmallInts(2, 2) :: LargeAndSmallInts(2147483646, 1) :: LargeAndSmallInts(
            3,
            2
          ) :: Nil
        )
      ...
      sqlContext.sparkContext
        .parallelize(
          DecimalData(1, 1) :: DecimalData(1, 2) :: DecimalData(2, 1) :: DecimalData(
            2,
            2
          ) :: DecimalData(3, 1) :: DecimalData(3, 2) :: Nil
        )
        .toDF()
    ...
    val df = sqlContext.sparkContext
      .parallelize(
        BinaryData("12".getBytes(StandardCharsets.UTF_8), 1) :: BinaryData(
          ...
          5
        ) :: BinaryData("122".getBytes(StandardCharsets.UTF_8), 3) :: BinaryData(
      ...
      sqlContext.sparkContext
        .parallelize(
          UpperCaseData(1, "A") :: UpperCaseData(2, "B") :: UpperCaseData(
            3,
            "C"
          ) :: UpperCaseData(4, "D") :: UpperCaseData(5, "E") :: UpperCaseData(
            6,
            "F"
          ) :: Nil
        )
  ...
  protected lazy val mapData: RDD[MapData] = {
    val rdd =
      sqlContext.sparkContext.parallelize(
        MapData(Map(1 -> "a1", 2 -> "b1", 3 -> "c1", 4 -> "d1", 5 -> "e1")) :: MapData(
          Map(1 -> "a2", 2 -> "b2", 3 -> "c2", 4 -> "d2")
        ) :: MapData(Map(1 -> "a3", 2 -> "b3", 3 -> "c3")) :: MapData(
          Map(1 -> "a4", 2 -> "b4")
  ...
  protected lazy val allNulls: DataFrame = {
    val df =
      sqlContext.sparkContext
        .parallelize(
          NullInts(null) :: NullInts(null) :: NullInts(null) :: NullInts(null) :: Nil
        )
        .toDF()
  ...
  protected lazy val nullStrings: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(
        NullStrings(1, "abc") :: NullStrings(2, "ABC") :: NullStrings(3, null) :: Nil
      )
      .toDF()
  ...
  protected lazy val complexData: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(
        ComplexData(Map("1" -> 1), TestData(1, "1"), Seq(1, 1, 1), true) :: ComplexData(
          Map("2" -> 2),
          TestData(2, "2"),
          Seq(2, 2, 2),
          false
  ...
  protected lazy val courseSales: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(
        CourseSales("dotNET", 2012, 10000) :: CourseSales("Java", 2012, 20000) :: CourseSales(
          "dotNET",
          2012,
          5000
        ) :: CourseSales("dotNET", 2013, 48000) :: CourseSales(
          "Java",
          2013,
          30000
        ) :: Nil

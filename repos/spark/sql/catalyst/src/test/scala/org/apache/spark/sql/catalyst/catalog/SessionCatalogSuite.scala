...
class SessionCatalogSuite extends SparkFunSuite {
  ...
  test("lookup table relation with alias") {
    ...
    assert(
      catalog
        .lookupRelation(TableIdentifier("tbl1", Some("db2")), alias = None) == relation
    )
    assert(
  ...
  test("list tables without pattern") {
    ...
    assert(
      catalog
        .listTables("db1")
        .toSet == Set(TableIdentifier("tbl1"), TableIdentifier("tbl4"))
    )
    assert(
  ...
  test("list functions") {
    ...
    assert(
      catalog
        .listFunctions("db1", "*")
        .toSet == Set(FunctionIdentifier("func1"), FunctionIdentifier("yes_me"))
    )
    assert(

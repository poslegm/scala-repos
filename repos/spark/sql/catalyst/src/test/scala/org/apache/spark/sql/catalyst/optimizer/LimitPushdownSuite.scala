...
class LimitPushdownSuite extends PlanTest {
  ...
  test("Union: limit to each side") {
    ...
    val unionCorrectAnswer = Limit(
      1,
      Union(LocalLimit(1, testRelation), LocalLimit(1, testRelation2))
    ).analyze
  ...
  test("Union: limit to each side with constant-foldable limit expressions") {
    ...
    val unionCorrectAnswer = Limit(
      2,
      Union(LocalLimit(2, testRelation), LocalLimit(2, testRelation2))
    ).analyze
    comparePlans(unionOptimized, unionCorrectAnswer)
  }
  ...
  test("Union: no limit to both sides if children having smaller limit values") {
    ...
    val unionCorrectAnswer = Limit(
      2,
      Union(testRelation.limit(1), testRelation2.select('d).limit(1))
    ).analyze
    comparePlans(unionOptimized, unionCorrectAnswer)
  }
    ...
    val unionCorrectAnswer =
      Limit(
        ...
        Union(
          ...
          LocalLimit(2, testRelation2.select('d))
        )
      ).analyze
    comparePlans(unionOptimized, unionCorrectAnswer)
  }

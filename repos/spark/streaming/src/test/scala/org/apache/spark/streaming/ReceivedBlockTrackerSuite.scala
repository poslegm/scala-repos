...
class ReceivedBlockTrackerSuite
  ...
  test("recovery and cleanup with write ahead logs") {
    ...
    tracker2.getBlocksOfBatchAndStream(
      batchTime2,
      streamId
    ) shouldEqual blockInfos2

    // Verify whether log has correct contents
    val expectedWrittenData2 =
      expectedWrittenData1 ++ Seq(
        createBatchAllocation(batchTime1, blockInfos1)
      ) ++ blockInfos2.map(BlockAdditionEvent) ++ Seq(
    ...
    tracker4.getBlocksOfBatchAndStream(
      batchTime1,
      streamId
    ) shouldBe empty // should be cleaned
    tracker4.getBlocksOfBatchAndStream(
      batchTime2,
      streamId
    ) shouldEqual blockInfos2
  ...
  def writeEventsManually(
      filePath: String,
      events: Seq[ReceivedBlockTrackerLogEvent]
  ): Unit = {
  ...
  def getWrittenLogData(
      logFiles: Seq[String] = getWriteAheadLogFiles

...
class ZkUtils(
  ...
  def getPartitionAssignmentForTopics(
    ...
    topics.foreach { topic =>
      ...
      debug(
        "Partition map for /brokers/topics/%s is %s".format(topic, partitionMap)
      )
  ...
  def getPartitionsUndergoingPreferredReplicaElection(
    ...
    val jsonPartitionListOpt = readDataMaybeNull(
      PreferredReplicaLeaderElectionPath
    )._1

...
object TestUtils extends Logging {
  ...
  def getMessageIterator(
      iter: Iterator[MessageAndOffset]
  ): Iterator[Message] = {
          ...
          topicAndPartitions.forall { tp =>
            val checkpoints = server.getLogManager().logDirs.map { logDir =>
              new OffsetCheckpoint(
                new File(logDir, "cleaner-offset-checkpoint")
              ).read()
            }
  ...
  def readString(
      buffer: ByteBuffer,
      encoding: String = Charset.defaultCharset.toString
  ): String = {

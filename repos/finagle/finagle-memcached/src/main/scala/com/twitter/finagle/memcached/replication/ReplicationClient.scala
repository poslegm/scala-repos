...
class BaseReplicationClient(
  ...
  def getsAll(
      keys: Iterable[String]
  ): Future[Map[String, ReplicationStatus[Option[(Buf, ReplicaCasUnique)]]]] = {
    val keySet = keys.toSet
  ...
  def checkAndSet(
      ...
      value: Buf,
      casUniques: Seq[Buf]
  ): Future[ReplicationStatus[CasResult]] =
  ...
  def add(
      ...
      expiry: Time,
      value: Buf
  ): Future[ReplicationStatus[JBoolean]] =

...
final class SortedSetClientServerIntegrationSuite
  ...
  test("ZCOUNT should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      ...
      assert(
        Await
          .result(client(ZCount(ZKEY, ZInterval.exclusive(1), ZInterval(3)))) == IntegerReply(
          2
        )
      )
    }
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await
          .result(client(ZInterStore("out", List(key, key2), Weights(2, 3)))) == IntegerReply(
          2
        )
      )
      assertMBulkReply(
      ...
      assert(
        Await
          .result(client(ZUnionStore("out", List(key, key2), Weights(2, 3)))) == IntegerReply(
          3
        )
      )
      assertMBulkReply(

...
final class SortedSetClientServerIntegrationSuite
  ...
  test("ZADD should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      assert(
        Await.result(
          client(ZAdd("zadd1", List(ZMember(1, "one"))))
        ) == IntegerReply(1)
      )
      assert(
        Await.result(
          client(ZAdd("zadd1", List(ZMember(2, "two"))))
        ) == IntegerReply(1)
      )
      assert(
        Await.result(
          client(ZAdd("zadd1", List(ZMember(3, "two"))))
        ) == IntegerReply(0)
      )
  ...
  test("ZCOUNT should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      ...
      assert(
        Await.result(
          client(ZCount(ZKEY, ZInterval.MIN, ZInterval.MAX))
        ) == IntegerReply(3)
      )
      assert(
        Await.result(

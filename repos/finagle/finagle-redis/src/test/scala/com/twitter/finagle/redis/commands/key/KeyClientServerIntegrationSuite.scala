...
final class KeyClientServerIntegrationSuite
  ...
  test(
    ...
    withRedisClient { client =>
      assert(
        Await.result(
          client(Exists(string2ChanBuf("nosuchkey")))
        ) == IntegerReply(0)
      )
    }
  }
  ...
  test(
    ...
    withRedisClient { client =>
      assert(
        Await.result(
          client(ExpireAt(boo, Time.now + 3600.seconds))
        ) == IntegerReply(0)
      )
    }
  }
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await.result(
          client(ExpireAt(foo, Time.now + 3600.seconds))
        ) == IntegerReply(1)
      )
    }
  }
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await.result(
          client(Move(baz, string2ChanBuf(toDb.toString)))
        ) == IntegerReply(1)
      )
    }
  }
  ...
  test(
    ...
    withRedisClient { client =>
      assert(
        Await.result(
          client(Persist(string2ChanBuf("nosuchKey")))
        ) == IntegerReply(0)
      )
    }
  }

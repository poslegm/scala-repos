...
class Netty3TransporterTest extends FunSpec with MockitoSugar with Eventually {
  describe("Netty3Transporter") {
    it("creates a Netty3Transporter instance based on Stack params") {
      ...
      assert(
        transporter.httpProxyCredentials == inputParams[Transporter.HttpProxy].credentials
      )
      ...
      assert(
        transporter.channelSnooper.nonEmpty == inputParams[Transport.Verbose].enabled
      )
    }
    ...
    describe("IdleStateHandler") {
      ...
      it(
        "is not added when neither channelReaderTimeout nor channelWriteTimeout are finite"
      ) {

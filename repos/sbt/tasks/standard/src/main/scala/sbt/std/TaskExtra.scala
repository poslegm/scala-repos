...
trait TaskExtra {
  ...
  final implicit def multInputTask[K[L[X]]](
      tasks: K[Task]
  ...
  final implicit def textPipeTask[Key](
      in: Task[_]
  )(implicit streams: Task[TaskStreams[Key]], key: Task[_] => Key): TextPipe =
    new TextPipe {
  ...
  final implicit def linesTask[Key](
      in: Task[_]
  )(implicit streams: Task[TaskStreams[Key]], key: Task[_] => Key): TaskLines =
    new TaskLines {

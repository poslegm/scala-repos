...
object BasicCommands {
  ...
  def append =
    Command(AppendCommand, Help.more(AppendCommand, AppendLastDetailed))(
      otherCommandParser
    ) { (s, arg) => s.copy(remainingCommands = s.remainingCommands :+ arg) }

  def setOnFailure =
    Command(OnFailure, Help.more(OnFailure, OnFailureDetailed))(
      otherCommandParser
    ) { (s, arg) => s.copy(onFailure = Some(arg)) }
  private[sbt] def compatCommands = Seq(
    ...
    Command.arb(s =>
      token(Compat.OnFailure, hide = const(true))
        .flatMap(x => otherCommandParser(s))
    ) { (s, arg) =>
  ...
  def continuous =
    Command(ContinuousExecutePrefix, continuousBriefHelp, continuousDetail)(
      ...
      withAttribute(
        ...
        "Continuous execution not configured."
      ) { w =>
        val repeat =

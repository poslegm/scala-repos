...
object Def extends Init[Scope] with TaskMacroExtra {
  ...
  def toISParser[T](p: Initialize[Parser[T]]): Initialize[State => Parser[T]] =
    p(toSParser)
  def toIParser[T](
      p: Initialize[InputTask[T]]
  ): Initialize[State => Parser[Task[T]]] =
    p(_.parser)
  ...
  def inputTaskDyn[T](
      t: Def.Initialize[Task[T]]
  ): Def.Initialize[InputTask[T]] = macro inputTaskDynMacroImpl[T]
  ...
  private[sbt] def dummyTask[T](name: String): Task[T] = {
    ...
    val base: Task[T] =
      newTask(
        sys.error(
          "Dummy task '" + name +
            "' did not get converted to a full task."
        )
      ) named name

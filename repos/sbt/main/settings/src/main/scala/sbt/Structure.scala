...
sealed abstract class SettingKey[T]
  ...
  protected[this] def make[S](other: Initialize[S], source: SourcePosition)(
      f: (T, S) => T
  ): Setting[T] = set((this, other)(f), source)
}
...
object Scoped {
  ...
  implicit def t2ToTable2[A, B](
      t2: (ScopedTaskable[A], ScopedTaskable[B])
  ): RichTaskable2[A, B] =
  ...
  ): RichTaskable9[A, B, C, D, E, F, G, H, I] =
  ...
  ): RichTaskable10[A, B, C, D, E, F, G, H, I, J] =
  ...
  sealed abstract class RichTaskables[K[L[x]]](
      final val keys: K[ScopedTaskable]
  )(implicit a: AList[K]) {
  ...
  implicit def t2ToApp2[A, B](
      t2: (Initialize[A], Initialize[B])
  ): Apply2[A, B] = new Apply2(t2)

...
object FullInstance
  ...
  def flatten[T](
      in: Initialize[Task[Initialize[Task[T]]]]
  ): Initialize[Task[T]] = {
    import Scoped._
  ...
  def flattenFun[S, T](
      in: Initialize[Task[S => Initialize[Task[T]]]]
  ): Initialize[S => Task[T]] = {
    import Scoped._
    (in, settingsData, Def.capturedTransformations) apply {
...
object TaskMacro {
  ...
  def settingAssignPure[T: c.WeakTypeTag](
      c: Context
  ...
  private[this] def transformMacroImpl(
      c: Context
  )(init: c.Tree)(newName: String): c.Tree = {
  ...
  private[this] def iInitializeMacro[M[_], T](
      c: Context
  )(t: c.Expr[T])(f: c.Expr[T] => c.Expr[M[T]])(implicit
      tt: c.WeakTypeTag[T],
      mt: c.WeakTypeTag[M[T]]
  ): c.Expr[Initialize[M[T]]] = {

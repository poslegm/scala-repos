...
object Load {
  ...
  def loaded(
      unit: sbt.BuildUnit
  ): (sbt.PartBuildUnit, List[ProjectReference]) = {
  ...
  private[this] def loadTransitive(
    ...
    def finalizeProject(p: Project, configFiles: Seq[File]): Project = {
      val loadedFiles =
        configFiles flatMap { f => memoSettings.get(f) }
  ...
  def referenced[PR <: ProjectReference](
      definitions: Seq[ProjectDefinition[PR]]
  ): Seq[PR] = definitions flatMap {
    _.referenced
  }

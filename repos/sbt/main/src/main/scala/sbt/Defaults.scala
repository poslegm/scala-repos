...
object Defaults extends BuildCommon {
  ...
  lazy val resourceConfigPaths = Seq(
    ...
    resourceGenerators <+=
      (
        ...
        resourceManaged
      ) map PluginDiscovery.writeDescriptors,
  ...
  lazy val configTasks =
    docTaskSettings(doc) ++ inTask(compile)(
    ...
    Def.task {
      ...
      Seq(
        new Tests.Group(
          "<default>",
          tests,
          if (fk) Tests.SubProcess(opts)
          else Tests.InProcess
        )
      )
    }
  private[this] def forkOptions: Initialize[Task[ForkOptions]] =
    (
      ...
      javaHome,
      connectInput
    ) map { (base, options, strategy, env, javaHomeDir, connectIn) =>
  ...
  private[this] lazy val inputTests0: Initialize[InputTask[Unit]] = {
    ...
    Def.inputTaskDyn {
      ...
      val modifiedOpts =
        Tests.Filters(filter(selected)) +: Tests.Argument(
          frameworkOptions: _*
        ) +: config.options
  ...
  lazy val packageConfig: Seq[Setting[_]] =
    inTask(packageBin)(
      Seq(
        packageOptions <<=
          (
            ...
            mainClass,
            packageOptions
          ) map { (name, ver, h, org, orgName, main, p) =>
      ...
      packageSrc,
      packageSrcMappings
    ) ++ packageTaskSettings(packageDoc, packageDocMappings) ++ Seq(
...
object Classpaths {
      ...
      Seq(
        ...
        classifiersModule <<=
          (
            ...
            loadedBuild,
            thisProjectRef
          ) map { (pid, sbtDep, classifiers, lb, ref) =>
  ...
  def deliverTask(
      config: TaskKey[DeliverConfiguration]
  ...
  def unmanagedDependencies: Initialize[Task[Classpath]] =
    (
      ...
      settingsData,
      buildDependencies
    ) flatMap unmanagedDependencies0
  ...
  def autoLibraryDependency(
    ...
    if (auto)
      modifyForPlugin(
        plugin,
        ModuleID(org, ScalaArtifacts.LibraryID, version)
      ) :: Nil
    else Nil
    ...
    repo match {
      ...
      case p: xsbti.PredefinedRepository =>
        p.id match {
          ...
          case unknown =>
            sys.error(
              "Unknown predefined resolver '" + unknown +
...
trait BuildCommon {
  ...
  def inputTask[T](
      f: TaskKey[Seq[String]] => Initialize[Task[T]]
  ...
  def overrideConfigs(
    ...
    val overridden =
      configurations map { conf => newByName.getOrElse(conf.name, conf) }
    val newConfigs =
      cs filter { c => !existingName(c.name) }

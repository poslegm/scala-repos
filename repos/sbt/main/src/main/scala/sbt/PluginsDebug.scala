...
private[sbt] object PluginsDebug {
    ...
    ps match {
      case PluginRequirements(
        ...
        val parts =
          ...
          ) :: indent(required(enablingPlugins.toList)) :: indent(
            needToDeactivate(deactivate)
          ) :: note(willAdd(plugin, extraEnabledPlugins.toList)) :: note(
  ...
  def definedKeys(
      available: List[AutoPlugin]
  ): Relation[AutoPlugin, AttributeKey[_]] = {
  ...
  private[this] def willRemovePlugins(
      base: AutoPlugin
  )(plugins: List[AutoPlugin]) =

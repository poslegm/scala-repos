...
abstract class MavenRepositoryResolver(settings: IvySettings)
    ...
    for {
      ...
      name <-
        Option(map.get(SbtPomExtraProperties.makeLicenseName(i)))
          .map(_.toString)
      url <-
        Option(map.get(SbtPomExtraProperties.makeLicenseUrl(i)))
          .map(_.toString)
  ...
  def getArtifactProperties(
    ...
    Option(
      dd.getExtraAttribute(PomExtraDependencyAttributes.ScalaVersionKey)
    ) foreach { sv => m.put(SbtPomExtraProperties.POM_SCALA_VERSION, sv) }
    ...
    art.getType match {
      ...
      case _                   =>
        ...
        art.getExtraAttribute(
          MavenRepositoryResolver.CLASSIFIER_ATTRIBUTE
        ) match {
          case null => None
          case c    => Some(c)
        }
        ...
        val artifacts = for ((art, file) <- t.artifacts) yield {
          Message.debug(
            s" - Publishing $art (${art.getType})(${art.getExtraAttribute(
              "classifier"
            )}) in [${art.getConfigurations.mkString(",")}] from $file"
          )
          new AetherArtifact(

...
object ScalaRefactoringUtil {
  ...
  def getExpression(
      ...
      startOffset: Int,
      endOffset: Int
  ): Option[(ScExpression, Array[ScType])] = {
  ...
  def getCompatibleTypeNames(
      myTypes: Array[ScType]
  ): util.LinkedHashMap[String, ScType] = {
  ...
  def enclosingContainer(parent: PsiElement): PsiElement = {
    Option(parent)
      .map(elem =>
        elem.firstChild.getOrElse(elem)

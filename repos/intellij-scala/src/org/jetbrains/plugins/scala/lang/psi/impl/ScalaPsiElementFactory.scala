...
class ScalaPsiElementFactoryImpl(manager: PsiManager)
  ...
  def createSubstitutor(
      map: util.Map[PsiTypeParameter, PsiType]
  ): PsiSubstitutor = ???
...
object ScalaPsiElementFactory {
  ...
  def createImplicitClassParamClauseFromTextWithContext(
    ...
    createElementWithContext(
      ...
      context,
      contextLastChild(context),
      ImplicitClassParamClause.parse(_)
    ) match {
      case clause: ScParameterClause => clause
      case _                         => null
    }
  ...
  def createBlockExpressionWithoutBracesFromText(
    ...
    createElement(
      text,
      manager,
      Block.parse(_, hasBrace = false, needNode = true)
    ) match {
            ...
            def buildText(param: ScParameter): String = {
              ...
              param.typeElement match {
                case Some(x) =>
                  ...
                  name + colon +
                    (if (param.isCallByNameParameter) arrow
                     else "") + typeText +
  ...
  def createReferenceFromText(
    ...
    createElementWithContext(
      ...
      context,
      child,
      StableId.parse(_, ScalaElementTypes.REFERENCE)
    ) match {
  ...
  def createXmlStartTag(
    ...
    createScalaFile(
      ...
      manager
    ).getFirstChild.getLastChild.getFirstChild.getFirstChild

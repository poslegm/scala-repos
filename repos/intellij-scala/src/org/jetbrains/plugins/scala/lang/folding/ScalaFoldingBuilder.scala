...
class ScalaFoldingBuilder extends CustomFoldingBuilder with PossiblyDumbAware {
      ...
      node.getPsi match {
        case sc: ScalaPsiElement =>
          (
            ...
            sc.getNextSiblingNotWhitespace
          ) match {
  ...
  private def isMultilineFuncBody(
      func: ScFunctionDefinition
  ): (Boolean, TextRange, String) = {
...
object TypeLambda {
  def unapply(
      psi: PsiElement
  ): Option[(String, ScTypeParamClause, ScTypeElement)] =
    psi match {

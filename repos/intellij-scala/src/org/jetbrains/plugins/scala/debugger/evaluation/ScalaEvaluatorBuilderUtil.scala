...
private[evaluation] trait ScalaEvaluatorBuilderUtil {
  ...
  def findContextClass(
      stopCondition: PsiElement => Boolean
  ): (PsiElement, Int) = {
...
object ScalaEvaluatorBuilderUtil {
    ...
    def unapply(elem: PsiElement): Option[ScFunction] = {
      ...
      def inner(element: PsiElement): Option[ScFunction] = {
        element match {
          case null => None
          case fun: ScFunction
              if isLocalFunction(fun) && !fun.parameters
                .exists(param => PsiTreeUtil.isAncestor(param, elem, false)) =>
            Some(fun)
  ...
  object privateTraitMethod {
    ...
    ): Option[(ScTrait, ScFunctionDefinition)] = {
      r.getElement match {

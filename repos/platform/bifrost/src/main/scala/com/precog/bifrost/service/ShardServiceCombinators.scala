...
object ShardServiceCombinators extends Logging {
  ...
  private def getSortOrder(
      request: HttpRequest[_]
  ): Validation[String, DesiredSortOrder] =
  ...
  private def getOffsetAndLimit(
      request: HttpRequest[_]
  ): ValidationNel[String, Option[(Long, Long)]] = {
...
trait ShardServiceCombinators
  ...
  def query[B](
    ...
    new DelegatingService[ByteChunk, ((APIKey, AccountDetails), Path) => Future[
      HttpResponse[B]
    ], ByteChunk, (APIKey, AccountDetails, Path, Query, QueryOptions) => Future[
      HttpResponse[B]
    ]] {
      val delegate = next
  ...
  def asyncQuery[B](
    ...
    new DelegatingService[ByteChunk, ((APIKey, AccountDetails)) => Future[
      ...
      val service = { (request: HttpRequest[ByteChunk]) =>
        ...
        delegate.service(
          request.copy(parameters = request.parameters + ('sync -> "async"))
        ) map { f => (cred: (APIKey, AccountDetails)) => f(cred, Path(path)) }
      }
  ...
  def requireAccount[A, B](accountFinder: AccountFinder[Future])(
      ...
      M: Monad[Future]
  ): HttpService[A, APIKey => Future[HttpResponse[B]]] = {
    val service0 =
      service map {
...
final class FindAccountService[A, B](accountFinder: AccountFinder[Future])(
    ...
    extends DelegatingService[A, APIKey => Future[B], A, Validation[
      String,
      (APIKey, AccountDetails)
    ] => Future[B]] {

...
object MongoAPIKeyManager extends Logging {
  ...
  )(implicit executor: ExecutionContext): (APIKeyManager[Future], Stoppable) = {
...
class MongoAPIKeyManager(
  ...
  def createGrant(
    ...
    database(
      insert(ng.serialize.asInstanceOf[JObject]).into(settings.grants)
    ) map { _ => logger.debug("Add complete for " + ng); ng }
  }

...
trait Solver extends parser.AST with typer.Binder {
  ...
  def solve(tree: Expr, sigma: Sigma)(
      partialPred: PartialFunction[Node, Boolean]
  ): Expr => Option[Expr] = {
    ...
    def solveBinary(tree: Expr, left: Expr, right: Expr)(
      ...
      if (inLeft && inRight) {
        val results =
          simplify(tree) map { xs => (inner(xs.head), xs) }
    ...
    def search(
        work: Set[List[Expr]],
        seen: Set[Expr],
        results: Set[List[Expr]]
    ): Set[List[Expr]] = {
      val filteredWork =
        work filterNot { xs => seen(xs.head) }
      // println("Examining: " + (filteredWork map { _.head } map printInfix))
      if (filteredWork.isEmpty) {
        results
      } else {
        val (results2, newWork) =
          filteredWork partition { xs => isSimplified(xs.head) }
        val newWorkLists =
          newWork flatMap { xs => possibilities(xs.head) map { _ :: xs } }
  ...
  def solveRelation(re: ComparisonOp, sigma: Sigma)(
      predicate: PartialFunction[Node, Boolean]
  ): Option[Expr] = {
  ...
  def solveComplement(
      c: Comp,
      sigma: Sigma
  ): PartialFunction[Node, Boolean] => Option[Expr] =
    c.child match {

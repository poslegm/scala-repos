...
trait Parser extends RegexParsers with Filters with AST {
  ...
  lazy val expr: Parser[Expr] =
    (id ~ "(" ~ formals ~ ")" ~ ":=" ~ expr ~ expr ^# {
      ...
      Assert(loc, e1, e2)
    } | """new\b""".r ~ expr ^# { (loc, _, e) => New(loc, e) } | relations ~ expr ^# {
      (loc, es, e) => buildDeepRelate(loc, es, e)
    } | namespacedId ^# { (loc, id) => Dispatch(loc, id, Vector()) } | ticId ^# TicVar | pathLiteral ^# {
      (loc, str) =>
        Dispatch(
          loc,
          LoadId,
      ...
      (loc, _) => UndefinedLit(loc)
    } | nullLiteral ^# { (loc, _) => NullLit(loc) } | "{" ~ properties ~ "}" ^# {
      (loc, _, ps, _) => ObjectDef(loc, ps)
    } | "[" ~ nullableActuals ~ "]" ^# { (loc, _, as, _) => ArrayDef(loc, as) } | expr ~ "." ~ propertyName ^# {
      (loc, e, _, p) => Descent(loc, e, p)
    } | expr ~ "@" ~ propertyName ^# { (loc, e, _, p) =>
      MetaDescent(loc, e, p)
    } | expr ~ "[" ~ expr ~ "]" ^# { (loc, e1, _, e2, _) => Deref(loc, e1, e2) } | namespacedId ~ "(" ~ actuals ~ ")" ^# {
      (loc, id, _, as, _) => Dispatch(loc, id, as)
    ...
    } | expr ~ "+" ~ expr ^# { (loc, e1, _, e2) => Add(loc, e1, e2) } | expr ~ "-" ~ expr ^# {
      (loc, e1, _, e2) => Sub(loc, e1, e2)
    } | expr ~ "*" ~ expr ^# { (loc, e1, _, e2) => Mul(loc, e1, e2) } | expr ~ "/" ~ expr ^# {
      (loc, e1, _, e2) => Div(loc, e1, e2)
    } | expr ~ "%" ~ expr ^# { (loc, e1, _, e2) => Mod(loc, e1, e2) } | expr ~ "^" ~ expr ^# {
      (loc, e1, _, e2) => Pow(loc, e1, e2)
    } | expr ~ "<" ~ expr ^# { (loc, e1, _, e2) => Lt(loc, e1, e2) } | expr ~ "<=" ~ expr ^# {
      (loc, e1, _, e2) => LtEq(loc, e1, e2)
    } | expr ~ ">" ~ expr ^# { (loc, e1, _, e2) => Gt(loc, e1, e2) } | expr ~ ">=" ~ expr ^# {
      (loc, e1, _, e2) => GtEq(loc, e1, e2)
    } | expr ~ "=" ~ expr ^# { (loc, e1, _, e2) => Eq(loc, e1, e2) } | expr ~ "!=" ~ expr ^# {
      (loc, e1, _, e2) => NotEq(loc, e1, e2)
    } | expr ~ "&" ~ expr ^# { (loc, e1, _, e2) => And(loc, e1, e2) } | expr ~ "|" ~ expr ^# {
      (loc, e1, _, e2) => Or(loc, e1, e2)
    } | "!" ~ expr ^# { (loc, _, e) => Comp(loc, e) } | """neg\b""".r ~ expr ^# {
      (loc, _, e) => Neg(loc, e)
    } | "(" ~ expr ~ ")" ^# { (loc, _, e, _) => Paren(loc, e) }) filter (precedence & arrayDefDeref & relateRelate)

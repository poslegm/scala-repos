...
trait Emitter
  ...
  def emit(expr: Expr): Vector[Instruction] = {
      ...
      case buckets.Group(origin, target, forest, dtrace) => {
        nextId { id =>
          ...
          emitBucketSpec(solve, forest, contextualDispatches, dispatches) >> prepareContext(
            context,
            dispatches
          ) { dispatches => emitExpr(target, dispatches) } >>
    ...
    def createJoins(
        ...
        op2: BinaryOperation
    ): (Set[Provenance], Vector[EmitterState]) = {
        ...
        (expr match {
          ...
          case expr @ ast.Solve(loc, _, body) =>
              ...
              btraces map {
                case (key, pairPaths) => {
                  val paths: List[List[Expr]] =
                    pairPaths map { pairs => pairs map { _._2 } }
            ...
            emitBucketSpec(expr, spec, contextualDispatches, dispatches) >> emitInstr(
              Split
            ) >> emitExpr(body, dispatches) >> emitInstr(Merge)

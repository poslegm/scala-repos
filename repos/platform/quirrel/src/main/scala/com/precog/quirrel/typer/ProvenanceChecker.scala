...
trait ProvenanceChecker extends parser.AST with Binder {
            ...
            if ((prov1 == InfiniteProvenance || prov2 == InfiniteProvenance) &&
              ...
              if (unified.isDefined) {
                ...
                (
                  ...
                  addedErrors
                ): (Provenance, Set[ProvConstraint], Set[Error])
              }
      ...
      val (provenance, errors, constr) = {
        if (pred.provenance == NullProvenance ||
          ...
          (leftUnified |@| rightUnified)(
            handleUnionLike(CondProvenanceDifferentLength)
          ) getOrElse {
  ...
  def unifyProvenance(
      relations: Map[Provenance, Set[Provenance]]
  )(p1: Provenance, p2: Provenance): Option[Provenance] =
    (p1, p2) match {
      ...
      this match {
        case UnifiedProvenance(_, _) =>
          findChildren(
            this,
            true
          ).toList sorted Provenance.order.toScalaOrdering reduceLeft UnifiedProvenance

        case ProductProvenance(_, _) =>
          findChildren(
            this,
            false
          ).toList sorted Provenance.order.toScalaOrdering reduceLeft ProductProvenance

        case CoproductProvenance(_, _) =>
          findChildren(
            this,
            false
          ).toList sorted Provenance.order.toScalaOrdering reduceLeft CoproductProvenance

        case DerivedUnionProvenance(_, _) =>
          findChildren(
            this,
            false
          ).toList sorted Provenance.order.toScalaOrdering reduceLeft DerivedUnionProvenance

        case DerivedIntersectProvenance(_, _) =>
          findChildren(
            this,
            false
          ).toList sorted Provenance.order.toScalaOrdering reduceLeft DerivedIntersectProvenance

        case DerivedDifferenceProvenance(_, _) =>
          findChildren(
            this,
            false
          ).toList sorted Provenance.order.toScalaOrdering reduceLeft DerivedDifferenceProvenance

...
object ParserSpecs
  ...
  "uncomposed expression parsing" should {
    ...
    "accept a solve expression with two tic variables" >> {
      ...
      "with And, Where" >> {
        parseSingle(
          "solve 'a, 'b ('a where true) & ('b where false)"
        ) must beLike {
          case Solve(
                _,
                Vector(TicVar(_, "'a"), TicVar(_, "'b")),
    ...
    "disambiguate solve and let" in {
      parseSingle(
        "solve 'a foo(b) := (solve 'c 'b + 'c) foo + 'a"
      ) must beLike {
        case Solve(
              _,
              Vector(TicVar(_, "'a")),
    ...
    "accept an object definition with a null property" in {
      parseSingle(
        "{ a: 1, b: 2, cafe: { foo: null }, star_BUckS: null }"
      ) must beLike {
        case ObjectDef(
              _,
              Vector(
    ...
    "accept an object definition with backtic-delimited properties" in {
      parseSingle(
        "{ `$see! what I can do___`: 1, `test \\` ing \\\\ with $%^&*!@#$ me!`: 2 }"
      ) must beLike {
        case ObjectDef(
              _,
              Vector(

...
trait BaselineComparisons {
  ...
  def compareWithBaseline(
      ...
      baseline: Map[TestPath, Statistics]
  ): Tree[(PerfTest, PerfDelta)] = {
    def rec(
        results: Tree[(PerfTest, Option[Statistics])],
        path: List[String]
    ): Tree[(PerfTest, PerfDelta)] =
            ...
            (obj \? "stats") match {
              case Some(stats) =>
                (for {
                  ...
                  JNum(variance) <-
                    stats \? "variance" flatMap
                      (_ -->? classOf[JNum])
                  JNum(stdDev) <-
                    stats \? "stdDev" flatMap
                      (_ -->? classOf[JNum])
  ...
  def createBaseline(result: Tree[(PerfTest, Option[Statistics])]): JValue = {
    ...
    def values(
        path: List[JString],
        test: Tree[(PerfTest, Option[Statistics])]
    ): List[JValue] = {
      test match {
        case Tree.Node((RunQuery(query), Some(stats)), _) =>
          JObject(
            JField("path", JArray(path)) :: JField(
              "query",
              JString(query)
            ) :: statsJson(stats)
          ) :: Nil

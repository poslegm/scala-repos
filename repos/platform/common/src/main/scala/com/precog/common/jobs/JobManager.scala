...
trait JobManager[M[+_]] { self =>
  ...
  def cancel(
      job: JobId,
      reason: String,
      cancelledAt: DateTime = new DateTime
  ): M[Either[String, Job]]
  ...
  def abort(
      job: JobId,
      reason: String,
      abortedAt: DateTime = new DateTime
  ): M[Either[String, Job]]
  ...
  def finish(
      job: JobId,
      finishedAt: DateTime = new DateTime
  ): M[Either[String, Job]]
  ...
  def expire(
      job: JobId,
      expiredAt: DateTime = new DateTime
  ): M[Either[String, Job]]
...
trait JobStateManager[M[+_]] { self: JobManager[M] =>
  ...
  protected def transition(job: JobId)(
      t: JobState => Either[String, JobState]
  ): M[Either[String, Job]]

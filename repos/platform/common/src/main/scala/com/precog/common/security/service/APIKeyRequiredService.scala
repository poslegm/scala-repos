...
trait APIKeyServiceCombinators extends HttpRequestHandlerCombinators {
  ...
  def invalidAPIKey[A](
      ...
      M: Monad[Future]
  ): String => Future[HttpResponse[A]] = { (msg: String) =>
    M.point(
...
) extends DelegatingService[A, Validation[String, APIKey] => Future[B], A, APIKey => Future[
      B
    ]] {
...
) extends DelegatingService[A, Future[B], A, Validation[String, APIKey] => Future[
      B
    ]]
    with Logging {
  val service = (request: HttpRequest[A]) => {

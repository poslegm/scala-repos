...
class AccessControlSpec extends Specification {
  ...
  "access control" should {
    ...
    "allow user accounts to read/reduce their data on any path" in {
      ...
      hasCapability(
        userAPIKey,
        Set(ReadPermission(Path("/user"), WrittenByAccount(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(ReducePermission(Path("/user"), WrittenByAccount(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(ReadPermission(Path("/other"), WrittenByAccount(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(ReducePermission(Path("/other"), WrittenByAccount(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(ReadPermission(Path("/"), WrittenByAccount(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(ReducePermission(Path("/"), WrittenByAccount(userAccountId)))
      ) must beTrue
    }
    ...
    "prevent user accounts from reading/reducing others data" in {
      ...
      hasCapability(
        userAPIKey,
        Set(ReadPermission(Path("/user"), WrittenByAccount(otherAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(ReducePermission(Path("/user"), WrittenByAccount(otherAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(ReadPermission(Path("/other"), WrittenByAccount(otherAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(ReducePermission(Path("/other"), WrittenByAccount(otherAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(ReadPermission(Path("/"), WrittenByAccount(otherAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(ReducePermission(Path("/"), WrittenByAccount(otherAccountId)))
      ) must beFalse
    }
    ...
    "allow user accounts to write/delete any data under their path" in {
      ...
      hasCapability(
        userAPIKey,
        Set(WritePermission(Path("/user"), WriteAs(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(DeletePermission(Path("/user"), WrittenByAccount(userAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(WritePermission(Path("/user"), WriteAs(otherAccountId)))
      ) must beTrue
      hasCapability(
        userAPIKey,
        Set(DeletePermission(Path("/user"), WrittenByAccount(otherAccountId)))
      ) must beTrue
    }
    ...
    "prevent user accounts from writing/deleting any data under another accounts path" in {
      ...
      hasCapability(
        userAPIKey,
        Set(WritePermission(Path("/other"), WriteAs(userAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(DeletePermission(Path("/other"), WrittenByAccount(userAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(WritePermission(Path("/other"), WriteAs(otherAccountId)))
      ) must beFalse
      hasCapability(
        userAPIKey,
        Set(DeletePermission(Path("/other"), WrittenByAccount(otherAccountId)))
      ) must beFalse
    }
    ...
    "prevent access via invalid API key" in {
      ...
      hasCapability(
        invalidAPIKey,
        Set(ReadPermission(Path("/"), WrittenByAny))
      ) must beFalse
      hasCapability(
        invalidAPIKey,
        Set(ReducePermission(Path("/"), WrittenByAny))
      ) must beFalse
      hasCapability(
        invalidAPIKey,

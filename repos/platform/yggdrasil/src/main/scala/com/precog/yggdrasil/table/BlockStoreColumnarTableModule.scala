...
trait BlockStoreColumnarTableModule[M[+_]]
  ...
  protected class MergeEngine[KeyType, BlockData <: BlockProjectionData[
    KeyType,
    Slice
  ]] {
  ...
  trait BlockStoreColumnarTableCompanion extends ColumnarTableCompanion {
    ...
    def load(
        ...
        apiKey: APIKey,
        tpe: JType
    ): EitherT[M, vfs.ResourceError, Table]
  }
  ...
  abstract class Table(slices: StreamT[M, Slice], size: TableSize)
    ...
    /**
      ...
      */
    def toInternalTable(
        limit: Int = yggConfig.maxSliceSize

    ...
    def plan0(
      ...
      t match {
        case Done =>
          ...
          def makeCols(
              pathToCol: Map[CPath, Set[Column]]
          ): Array[(CPath, Column)] = {
...
object CPathTraversal {
  ...
  private object CPathPosition {
    ...
    def disjoint(
        ...
        c2: List[CPathPosition]
    ): List[List[CPathPosition]] = {
          ...
          case (CPathRange(ns1, l1, r1) :: ps, CPathRange(ns2, l2, r2) :: qs)
            ...
            loop(
              ...
              CPathRange(
                ns1 ++ ns2,
                math.max(l1, l2),
                ^(r1, r2)(math.min(_, _)) orElse r1 orElse r2
              ) :: is,
              rss1
            )
    ...
    def disjointOrder(paths: List[CPath]): List[List[CPathPosition]] = {
      ...
      val pq = mutable.PriorityQueue[List[CPathPosition]](
        ...
        implicitly[
          scalaz.Order[List[CPathPosition]]
        ].reverseOrder.toScalaOrdering
      }

      @tailrec
      def rec(
          a: List[CPathPosition],
          ts: List[List[CPathPosition]]
      ): List[List[CPathPosition]] = {
        if (!pq.isEmpty) {
          val b = pq.dequeue()
          if (intersect(a, b)) {

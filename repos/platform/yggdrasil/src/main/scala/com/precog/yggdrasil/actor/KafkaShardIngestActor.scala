...
object FilesystemIngestFailureLog {
  ...
  def apply(
    ...
    def readAll(
        ...
        into: Map[EventMessage, LogRecord]
    ): Map[EventMessage, LogRecord] = {
    ...
    if (logFiles.isEmpty)
      ...
      val reader = new BufferedReader(
        new FileReader(
          logFiles
            .maxBy(_.getName.substring(FilePrefix.length).dropRight(4).toLong)
        )
      )
      try {
  ...
  private def readRemote(fromCheckpoint: YggCheckpoint): Future[
    ...
    def buildBatch(
      ...
      input match {
        ...
        case (
              offset,
              event @ IngestMessage(_, _, _, records, _, _, _)
            ) :: tail =>
          val newCheckpoint =
            if (records.isEmpty) {
        ...
        case (
              offset,
              ar @ ArchiveMessage(_, _, _, EventId(pid, sid), _)
            ) :: tail =>

...
trait ActorVFSModule extends VFSModule[Future, Slice] {
  ...
  class ResourceBuilder(
    ...
    ): IO[ResourceError \/ NIHDBResource] = {
      for {
    ...
    def createBlob[M[+_]](
        ...
        M: Monad[M],
        IOT: IO ~> M
    ): M[ResourceError \/ FileBlobResource] = {
      def write(
          out: FileOutputStream,
          size: Long,
          stream: StreamT[M, Array[Byte]]
      ): M[ResourceError \/ Long] = {
        ...
        blobResult <-
          IOT {
            writeResult traverse { size =>
              ...
              ) map { _ => FileBlobResource(file, metadata) }
            }
          }
      } yield blobResult
    }
  ...
  class ActorVFS(
    ...
    def readResource(
        path: Path,
        version: Version
    ): EitherT[Future, ResourceError, Resource] = {
    ...
    def findDirectChildren(
        path: Path
    ): EitherT[Future, ResourceError, Set[PathMetadata]] = {
      implicit val t = projectionReadTimeout
      EitherT {
    ...
    def findPathMetadata(
        path: Path
    ): EitherT[Future, ResourceError, PathMetadata] = {
      implicit val t = projectionReadTimeout
      EitherT {
  ...
  final class PathManagerActor(
    ...
    private def openResource(
        version: UUID
    ): EitherT[IO, ResourceError, Resource] = {
    ...
    def processEventMessages(
      ...
      def openNIHDB(
          version: UUID
      ): EitherT[IO, ResourceError, ProjectionResource] = {
    ...
    def receive = {
      case ReceiveTimeout =>
        ...
        val quiesce =
          versions.values.toStream collect {

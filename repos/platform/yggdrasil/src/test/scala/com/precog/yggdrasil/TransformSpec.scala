...
trait TransformSpec[M[+_]]
  ...
  def testDeepMap1CoerceToDouble = {
    val data: Stream[JValue] =
      Stream(
        JObject(
          JField("value", JNum(12)) :: JField("key", JArray(JNum(1) :: Nil)) :: Nil
        ),
        JObject(
          JField("value", JNum(34.5)) :: JField("key", JArray(JNum(2) :: Nil)) :: Nil
        ),
        JObject(
          JField("value", JNum(31.9)) :: JField("key", JArray(JNum(3) :: Nil)) :: Nil
        ),
        JObject(
        ...
        JObject(
          JField("value", JNum(20)) :: JField("key", JArray(JNum(4) :: Nil)) :: Nil
        )
      )
  ...
  def testMap2ArrayObject = {
    val data: Stream[JValue] = Stream(
      ...
      JObject(
        JField("value1", JNum(20)) :: JField("value2", JArray(JNum(1) :: Nil)) :: Nil
      ),
      JObject(
  ...
  def checkEqualSelf = {
    ...
    check { (sample: SampleData) =>
      ...
      results.copoint must_==
        (Stream.tabulate(sample.data.size) { _ => JBool(true) })
    }
  }
  ...
  def checkEqualSelfArray = {
    ...
    val results = toJson(
      table
        .cross(table2)(InnerObjectConcat(wrappedIdentitySpec, wrappedValueSpec))
    )
  ...
  def checkWrapObject = {
    ...
    check { (sample: SampleData) =>
      ...
      val expected =
        sample.data map { jv => JObject(JField("foo", jv) :: Nil) }
  ...
  def checkObjectDelete = {
    ...
    def randomDeletionMask(
        schema: CValueGenerators.JSchema
    ): Option[JPathField] = {
      Random
        .shuffle(schema)
        .headOption
  ...
  def testIsType(sample: SampleData) = {
    ...
    val expected0 =
      schemas map { schema => Schema.subsumes(schema, jtpe) }
    val expected =
      expected0 map {
        case true  => JTrue
        case false => JFalse
    ...
    val expected = Stream(
      JObject(
        JField("key", JArray(JNum(1) :: Nil)) :: JField(
          "value",
          JArray(JUndefined :: JBool(true) :: Nil)
        ) :: Nil
      )
    )
  ...
  def testTrivialScan = {
    val data =
      JObject(
        ...
        JField("value", JString("")) :: JField("key", JArray(JNum(2) :: Nil)) :: Nil
      ) #:: Stream.empty
  ...
  def testHetScan = {
    val data =
      JObject(
        ...
        JField("value", JNum(10)) :: JField("key", JArray(JNum(2) :: Nil)) :: Nil
      ) #:: JObject(
  ...
  def testDerefObjectDynamic = {
    val data =
      JObject(JField("foo", JNum(1)) :: JField("ref", JString("foo")) :: Nil) #:: JObject(
        ...
        JField("baz", JNum(3)) :: JField("ref", JString("baz")) :: Nil
      ) #:: Stream
        .empty[JValue]

...
class CSVIngestProcessing(
  ...
  ): ValidationNel[String, IngestProcessor] = {
  ...
  final class IngestProcessor(
    ...
    @tailrec final def readBatch(
        reader: CSVReader,
        batch: Vector[Array[String]]
    ): (Boolean, Vector[Array[String]]) = {
          ...
          case (done, batch) =>
            if (batch.isEmpty) {
              ...
              ingestStore.store(
                ...
                jobId,
                streamRef.terminate
              ) flatMap { _ => M.point(BatchResult(total, ingested, errors)) }
            } else {

...
trait StatsLibSpecs[M[+_]]
  ...
  "homogenous sets" should {
    ...
    "median with even number of elements" >> {
      ...
      val input = dag.Morph1(
        Median,
        dag.AbsoluteLoad(Const(CString("/hom/numbers5"))(line))(line)
      )(line)
  ...
  "heterogenous sets" should {
    ...
    "mode in the case there is more than one" >> {
      ...
      val input = dag.Morph1(
        Mode,
        dag.AbsoluteLoad(Const(CString("/het/random"))(line))(line)
      )(line)
    ...
    "simple exponential smoothing" in {
      ...
      val left = Join(
        ...
        Join(
          ...
          Cross(None),
          const("smooth"),
          Join(DerefObject, Cross(None), data, const("weight"))(line)
        )(line),
        Join(
          WrapObject,
          Cross(None),
          const("by"),
          Join(DerefObject, Cross(None), data, const("height"))(line)
        )(line)
      )(line)
    ...
    "compute covariance" in {
      ...
      val input = dag.Morph2(
        ...
        Join(
          ...
          Cross(None),
          dag.AbsoluteLoad(
            Const(CString("het/heightWeightAcrossSlices"))(line)
          )(line),
          Const(CString("height"))(line)
        )(line),
        Join(

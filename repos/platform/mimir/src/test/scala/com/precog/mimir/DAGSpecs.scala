...
object DAGSpecs extends Specification with DAG with FNDummyModule {
  ...
  "dag decoration" should {
    ...
    "recognize a new instruction" in {
      decorate(
        Vector(Line(1, 1, ""), PushNum("5"), Map1(instructions.New))
      ) mustEqual Right(
  ...
  "stream validation" should {
    ...
    "detect stack underflow" in {
      ...
      "map2_match" >> {
...

        {
          val instr = Map2Match(Add)
          decorate(
            Vector(Line(1, 1, ""), PushTrue, Map1(Comp), instr, Map2Match(Sub))
          ) mustEqual Left(StackUnderflow(instr))
        }
      }
      ...
      "map2_cross" >> {
...

        {
          val instr = Map2Cross(Add)
          decorate(
            Vector(Line(1, 1, ""), PushTrue, Map1(Comp), instr, Map2Cross(Sub))
          ) mustEqual Left(StackUnderflow(instr))
        }
      }
      ...
      "filter_match" >> {
...

        {
          val instr = FilterMatch
          decorate(
            Vector(Line(1, 1, ""), PushTrue, PushTrue, Map2Match(Add), instr)
          ) mustEqual Left(StackUnderflow(instr))
        }
      }
      ...
      "swap" >> {
...

        {
          val instr = Swap(2)
          decorate(
            Vector(Line(1, 1, ""), PushTrue, PushTrue, instr)
          ) mustEqual Left(StackUnderflow(instr))
        }
    ...
    "reject multiple stack values at end" in {
      ...
      decorate(
        Vector(Line(1, 1, ""), PushTrue, PushFalse, PushNum("42"))
      ) mustEqual Left(MultipleStackValuesAtEnd)
      decorate(
        Vector(
          Line(1, 1, ""),
          PushTrue,
    ...
    "reject merge with deepened stack" in {
      ...
      ) mustEqual Left(MergeWithUnmatchedTails)
    }

...
trait ReductionLibSpecs[M[+_]]
  ...
  def parseDateTimeFuzzy(time: String) =
    Operate(
      ...
      dag.AbsoluteLoad(Const(CString(time))(line))(line)
    )(line)
  ...
  "reduce homogeneous sets" >> {
    ...
    "count het numbers" >> {
      val input =
        dag.Reduce(
          Count,
          dag.AbsoluteLoad(Const(CString("/hom/numbersHet"))(line))(line)
        )(line)
    ...
    "forall" >> {
      val input = dag.Reduce(
        Forall,
        dag.IUI(true, Const(CTrue)(line), Const(CFalse)(line))(line)
      )(line)

  ...
  trait TransSpecable extends EvaluatorMethods {
    ...
    def isTransSpecable(to: DepGraph, from: DepGraph): Boolean =
      foldDownTransSpecable(to, Some(from))(new TransSpecableFold[Boolean] {
        ...
        def binOp(
            node: Join
        )(leftParent: Boolean, rightParent: => Boolean, op: BinaryOperation) =
    ...
    def transFold[N[+_]: Monad, S](
      ...
      new TransSpecableFold[N[S]] {
        ...
        def EqualLiteral(
            node: Join
        )(parent: N[S], value: RValue, invert: Boolean) =
        ...
        def binOp(
          ...
          for {
            ...
            result <- if (al == ar) {
                       set(
                         pl,
                         (transFromBinOp(op, MorphContext(ctx, node))(l, r), al)
                       )
                     } else {

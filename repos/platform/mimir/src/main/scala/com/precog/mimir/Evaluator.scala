  ...
  abstract class EvaluatorLike[N[+_]](N0: Monad[N])(implicit
    ...
    def fullRewriteDAG(
      ...
      stagedRewriteDAG(
        optimize,
        ctx
      ) andThen (orderCrosses _) andThen composeOptimizations(
        optimize,
    ...
    def eval(
      ...
      def prepareEval(
        ...
        def join(
              ...
              val components =
                for (i <- ids)
                  yield trans.WrapArray(
                    DerefArrayStatic(SourceKey.Single, CPathIndex(i))
                  ): TransSpec1
          ...
          def join0(
              ...
              pendingTableRight: PendingTable
          ): M[PendingTable] = {
            ...
            case dag.Morph2(mor, left, right) =>
              ...
              joined flatMap {
                case (morph1, PendingTable(joinedTable, _, _, sort)) =>
                  transState liftM mn(
                    morph1(joinedTable, MorphContext(ctx, graph))
                  ) map { table =>
                    PendingTable(
                      table,
                      graph,
              ...
              if (parent.valueKeys contains id) {
                ...
                for {
                  ...
                  result = {
                    ...
                    val oldSortFields =
                      parent.valueKeys map { id0 => CPathField("sort-" + id0) }
      ...
      def fullEval(
          graph: DepGraph,
          splits: Map[Identifier, Int => N[Table]],
          parentSplits: List[Identifier]
      ): StateT[N, EvaluatorState, Table] = {
    ...
    private[this] def listStagingPoints(
        ...
        acc: List[dag.StagingPoint] = Nil
    ): List[dag.StagingPoint] = {
    ...
    private def referencesOnlySplit(
        parentSplits: List[Identifier]
    ...
    private def findCommonality(nodes: Set[DepGraph]): Option[DepGraph] = {
      ...
      if (nodes.size == 1) {
        ...
        val kernels =
          nodes map { n => Kernel(Set(n), Set(n)) }
    ...
    private def disjunctiveEquals(
        specs: (IdentitySpec, IdentitySpec)
    ): Boolean =
      specs match {

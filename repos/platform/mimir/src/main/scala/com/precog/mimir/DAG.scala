    ...
    def loop(
            ...
            case instr @ (instructions.IIntersect | instructions.IUnion) =>
              continue {
                case Right(right) :: Right(left) :: tl =>
                  Right(
                    Right(
                      IUI(instr == instructions.IUnion, left, right)(loc)
                    ) :: tl
                  )
                case Left(_) :: _ | _ :: Left(_) :: _ =>
                  Left(OperationOnBucket(instr))
      ...
      val tail: Option[Trampoline[Either[StackError, DepGraph]]] =
        stream.headOption map {
          ...
          case PushKey(id) => {
            val openPoss =
              splits find { open => findGraphWithId(id)(open.spec).isDefined }
            openPoss map { open =>
              loop(
                loc,
  ...
  object dag {
    ...
    case class Cond(
      ...
      val peer =
        IUI(
          ...
          Filter(rightJoin, right, Operate(Comp, pred)(loc))(loc)
        )(loc)
    ...
    case class IdentityMatch(left: DepGraph, right: DepGraph) {
      ...
      private def findMatch(
          specs: Vector[IdentitySpec]
      )(spec: (IdentitySpec, Int)): Option[(IdentitySpec, (Int, Int))] = {
        val idx =
          specs indexWhere { s => intersects(spec._1, s) }
        if (idx < 0) None

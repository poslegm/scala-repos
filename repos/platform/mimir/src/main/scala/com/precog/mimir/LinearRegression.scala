  ...
  trait LinearRegressionLib
    ...
    object MultiLinearRegression
      ...
      def insertZeroAt(
        ...
        val indices =
          indices0 filter { i => (i >= 0) && (i < vlength + indices0.length) }
      ...
      def removeAt(
        ...
        val vlength = values.length
        val indices =
          indices0 filter { i => (i >= 0) && (i < vlength) }
        ...
        val values =
          features map {
            case c: HomogeneousArrayColumn[_]
              ...
              val mapped =
                range.toArray filter { r => c.isDefinedAt(r) } map { i =>
      ...
      def stdErrorReducer(acc: CoeffAcc): Reducer[StdErrorAcc] =
        new Reducer[StdErrorAcc] {
          def reduce(schema: CSchema, range: Range): StdErrorAcc = {
            ...
            val actualY0 =
              y0 map { arr => (new Matrix(Array(arr))).transpose() }

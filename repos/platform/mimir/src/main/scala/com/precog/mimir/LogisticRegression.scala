  ...
  trait LogisticRegressionLib
    ...
    object LogisticRegression
      ...
      def reduceDouble(seq0: Seq[ColumnValues]): Result = {
        val seq =
          seq0 filter { arr => arr.last == 0 || arr.last == 1 }
            ...
            val result: Set[Result] =
              features map {
                case c: HomogeneousArrayColumn[_]
                  ...
                  val mapped =
                    range filter { r => c.isDefinedAt(r) } map { i =>
      ...
      def extract(res: Result, jtype: JType): Table = {
        ...
        res map {
          case seq => {
            ...
            val res =
              finalTheta map { v => RObject(Map("estimate" -> CNum(v))) }
      ...
      private val morph1 = new Morph1Apply {
        def apply(table0: Table, ctx: MorphContext): M[Table] = {
          ...
          val sampleTables: M[Seq[Table]] =
            specs flatMap { seq => table.sample(10000, seq) }

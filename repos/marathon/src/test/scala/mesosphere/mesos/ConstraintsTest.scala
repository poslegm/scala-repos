  ...
  test("Select tasks to kill for multiple group by works") {
    ...
    val tasks =
      0.to(9)
        ...
        .to(19)
        .map(num =>
          makeSampleTask(s"$num", Map("rack" -> "rack-1", "color" -> "green"))
        ) ++ 20
        .to(29)
        .map(num =>
    ...
    result.count(
      _.agentInfo.attributes.exists(_.getText.getValue == "rack-1")
    ) should be(10)
    result.count(
      _.agentInfo.attributes.exists(_.getText.getValue == "rack-2")
    ) should be(10)
    result.count(
      _.agentInfo.attributes.exists(_.getText.getValue == "blue")
    ) should be(10)
    result.count(
      _.agentInfo.attributes.exists(_.getText.getValue == "green")
    ) should be(10)
  }

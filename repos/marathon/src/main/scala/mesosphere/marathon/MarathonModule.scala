...
class MarathonModule(conf: MarathonConf, http: HttpConf, zk: ZooKeeperClient)
  ...
  def provideGroupManager(
      @Named(
        ModuleNames.SERIALIZE_GROUP_UPDATES
  ...
  private[this] def entityStore[
      ...
      prefix: String,
      newState: () => T
  )(implicit ct: ClassTag[T]): EntityStore[T] = {

...
object Templates {
    ...
    s3Settings ++ Seq(
        ...
        val mappings: Seq[(File, File)] = preparedTemplates.flatMap {
          template =>
            (template.***.filter(!_.isDirectory) x relativeTo(template)).map {
              f => (f._1, syncTemplateDirValue / template.getName / f._2)
            }
        }
      ...
      prepareTemplates := {
        ...
        streams.value.log.info(
          "Preparing templates for Play " + params("PLAY_VERSION") + " with Scala " + params(
            "SCALA_VERSION"
          )
        )
          ...
          case (original, file) =>
            ...
            val templateParams: Map[String, String] = {
              ...
              val rebasedFile = (file relativeTo outDir).getOrElse(
                sys
                  .error(s"Can't rebase prepared template $file to dir $outDir")
              )
  ...
  private def templatesParser(
      state: State
  ): Parser[(Seq[TemplateSources], String)] = {
    import Parser._
    import Parsers._

...
object DiscoveryClientSpec extends Specification with Mockito {
  ...
  "Discovery normalization" should {
    ...
    "normalize port" in {
      normalize(
        "HTTP://EXAMPLE.COM:80/A/B?Q=Z#"
      ) must be equalTo "http://example.com/A/B?Q=Z"
      normalize(
        "https://example.com:443"
      ) must be equalTo "https://example.com/"
    }
    "normalize paths" in {
      normalize(
        "http://example.com//a/./b/../b/c/"
      ) must be equalTo "http://example.com/a/b/c/"
      normalize(
        "http://example.com?bla"
      ) must be equalTo "http://example.com/?bla"
    }
  }
  ...
  "Discovery normalization" should {
    ...
    "normalize URLs according to he OpenID example in the spec" in {
      ...
      "No trailing slash is added to non-empty path components" in {
        ...
        ) must be equalTo "http://example.com/user"
      }
    ...
    "normalize URLs according to he OpenID 2.0 spec" in {
      ...
      "If the URL contains a fragment part, it MUST be stripped off together with the fragment delimiter character \"#\"." in {
        normalize(
          "example.com#thefragment"
        ) must be equalTo "http://example.com/"
        normalize(
          "example.com/#thefragment"
        ) must be equalTo "http://example.com/"
        normalize(
          "http://example.com#thefragment"
        ) must be equalTo "http://example.com/"
        normalize(
          "https://example.com/#thefragment"
        ) must be equalTo "https://example.com/"
      }
    }
  }

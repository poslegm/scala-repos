...
class Logger$ColoredLevel extends ColoredLevel {
  ...
  override def start(): Unit = {
    ...
    addError(
      s"You are using the deprecated ${this.getClass.getName} in $configLocation, please use ${classOf[ColoredLevel].getName} instead. See $migrationDocs for more information."
    )
  }
}

...
object RouterSpec extends PlaySpecification {
  ...
  "bind boolean parameters" in {
    "from the query string" in new WithApplication() {
      ...
      contentAsString(
        route(implicitApp, FakeRequest(GET, "/take-bool?b=1")).get
      ) must equalTo("true")
      contentAsString(
        route(implicitApp, FakeRequest(GET, "/take-bool?b=0")).get
      ) must equalTo("false")
    }
    "from the path" in new WithApplication() {
      ...
      // Bind boolean values from 1 and 0 integers too
      contentAsString(
        route(implicitApp, FakeRequest(GET, "/take-bool-2/1")).get
      ) must equalTo("true")
      contentAsString(
        route(implicitApp, FakeRequest(GET, "/take-bool-2/0")).get
      ) must equalTo("false")
    }
  }

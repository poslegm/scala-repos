...
object AllowedHostsFilterSpec extends PlaySpecification {
  ...
  "the allowed hosts filter" should {
    ...
    "not allow malformed ports" in withApplication(
      ...
      status(
        request("addons.mozilla.org:@passwordreset.net")
      ) must_== BAD_REQUEST
      status(
        request("addons.mozilla.org: www.securepasswordreset.com")
      ) must_== BAD_REQUEST
    }
    ...
    "validate hosts in absolute URIs" in withApplication(
      ...
      status(
        request(
          ...
          "https://addons.mozilla.org/en-US/firefox/users/pwreset"
        )
      ) must_== OK
      status(
        request(
          "addons.mozilla.org",
          "https://www.securepasswordreset.com/en-US/firefox/users/pwreset"
        )
      ) must_== BAD_REQUEST
    }

...
object IterateesSpec
      ...
      mustExecute(1) { flatMapEC =>
        await(
          Done(3)
            ...
            .unflatten
        ) must equalTo(Step.Done(6, Input.Empty))
      }
    }
      ...
      mustExecute(1) { flatMapEC =>
        await(
          Done(3, Input.El("remaining"))
            ...
            .unflatten
        ) must equalTo(Step.Done(6, Input.El("remaining")))
      }
    }
      ...
      mustExecute(1) { flatMapEC =>
        await(
          Done(3, Input.El("remaining"))
            ...
            .unflatten
        ) must equalTo(Step.Error("error", Input.El("bad")))
      }
    }
      ...
      mustExecute(1) { flatMapEC =>
        await(
          Done(3, Input.El(List(1, 2)))
            ...
            .unflatten
        ) must equalTo(Step.Done(4, Input.El(List(1, 2, 3, 4))))
      }
    }
  }
      ...
      mustExecute(0) { flatMapEC =>
        await(
          Error("msg", Input.El("bad"))
            ...
            .unflatten
        ) must equalTo(Step.Error("msg", Input.El("bad")))
      }
    }
  }
    ...
    "map the final iteratee's result (with mapM)" in {
      mustExecute(4, 1) { (foldEC, mapEC) =>
        await(
          Enumerator(1, 2, 3, 4) |>>> Iteratee
            .fold[Int, Int](0)(_ + _)(foldEC)
            .mapM(x => Future.successful(x * 2))(mapEC)
        ) must equalTo(20)
      }
    }
  }
  ...
  "Iteratee.consume" should {
    ...
    "return its concatenated input" in {
      ...
      await(
        Enumerator.enumerateSeq1(s) |>>> Iteratee.consume[List[Int]]()
      ) must equalTo(r)
    }
  }
  ...
  "Iteratee.getChunks" should {
    ...
    "return its input as a list" in {
      ...
      await(
        Enumerator.enumerateSeq1(s) |>>> Iteratee.getChunks[Int]
      ) must equalTo(s)
    }
  }
  ...
  "Iteratee.ignore" should {
    ...
    "never throw an OutOfMemoryError when consuming large input" in {
      ...
      await(
        Enumerator.enumerate(iterator) |>>> Iteratee.ignore[Array[Byte]]
      ) must_==
        (())
    }
  }

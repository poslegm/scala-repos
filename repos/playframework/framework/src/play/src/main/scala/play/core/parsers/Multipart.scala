...
object Multipart {
    ...
    BodyParser { request =>
      partParser(maxMemoryBufferSize) {
        ...
        val multipartAccumulator = Accumulator(
          Sink.fold[Seq[Part[A]], Part[A]](Vector.empty)(_ :+ _)
        ).mapFuture { parts =>
    ...
    case FileInfo(partName, filename, contentType) =>
      ...
      Accumulator(
        StreamConverters.fromOutputStream(() =>
          new java.io.FileOutputStream(tempFile.file)
        )
      ).map { _ => FilePart(partName, filename, contentType, tempFile) }
  }
  ...
  private final class BodyPartParser(
    ...
    def continue(input: ByteString, offset: Int)(
        next: (ByteString, Int) ⇒ StateResult
    ): StateResult = {

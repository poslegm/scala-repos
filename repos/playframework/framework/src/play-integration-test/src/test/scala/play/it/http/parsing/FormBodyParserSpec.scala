  ...
  "The form body parser" should {
    ...
    def parse[A, B](body: B, bodyParser: BodyParser[A])(implicit
      ...
      await(
        bodyParser(
          FakeRequest()
            .withHeaders(writeable.contentType.map(CONTENT_TYPE -> _).toSeq: _*)
    ...
    "not bind erroneous body" in new WithApplication() {
      parse(
        Json.obj("age" -> "Alice"),
        BodyParsers.parse.form(userForm)
      ) must beLeft(Results.BadRequest)
    }
    ...
    "allow users to override the error reporting behaviour" in new WithApplication() {
      ...
      parse(
        ...
        BodyParsers.parse.form(
          userForm,
          onErrors = (form: Form[User]) => Results.BadRequest(form.errorsAsJson)
        )

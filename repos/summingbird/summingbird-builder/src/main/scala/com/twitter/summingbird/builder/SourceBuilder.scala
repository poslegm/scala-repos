...
case class SourceBuilder[T: Manifest] private (
  ...
  def flatMapBuilder[U: Manifest](
      newFlatMapper: FlatMapper[T, U]
  ): SourceBuilder[U] =
    flatMap(newFlatMapper(_))

  def write[U](sink: CompoundSink[U])(
      conversion: T => TraversableOnce[U]
  ...
  def leftJoin[K, V, JoinedValue](
      service: CompoundService[K, JoinedValue]
  )(implicit
      ev: T <:< (K, V),
      keyMf: Manifest[K],
      valMf: Manifest[V],
    ...
    val cb = env match {
      case scalding: ScaldingEnv =>
        ...
        val newNode = OptionalUnzip2[Scalding, Storm]()(node)._1
          .map { p => Producer.evToKeyed(p.name(id)).sumByKey(batchSetStore) }
          .getOrElse(
            sys.error(
      ...
      case storm: StormEnv =>
        ...
        val newNode = OptionalUnzip2[Scalding, Storm]()(node)._2
          .map { p => Producer.evToKeyed(p.name(id)).sumByKey(givenStore) }
          .getOrElse(
            sys.error(

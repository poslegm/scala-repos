...
object TestGraphs {
  ...
  private def loopJoinInScala[K: Ordering, U, V: Monoid](
    ...
    (K, List[(Option[(Long, (U, Option[V]))], Option[(Long, (Option[V], V))])])
  ] = {
    leftAndRight
      .groupBy(_._1)
      .mapValues {
  ...
  def singleStepMapKeysInScala[T, K1, K2, V: Monoid](
      ...
      fnB: K1 => TraversableOnce[K2]
  ): Map[K2, V] =
    MapAlgebra.sumByKey(
      source
        .flatMap(fnA)
        .flatMap { x => fnB(x._1).map((_, x._2)) }
    )
  ...
  def leftJoinWithStoreInScala[
      ...
      postJoinFn: (
          (Long, (K, (U, Option[JoinedU])))
      ) => TraversableOnce[(Long, (K, V))]
  ...
  def leftJoinWithDependentStoreJoinFanoutJob[P <: Platform[
    P
  ], T1, V1: Monoid, U, K, V: Monoid](
      source1: Producer[P, T1],
      storeAndService: P#Store[K, V] with P#Service[K, V],
      store: P#Store[K, V1]
  )(
      simpleFM1: T1 => TraversableOnce[(K, U)]
  ...
  def realJoinTestJob[P <: Platform[
    P
  ], T1, T2, T3, T4, K1, K2, U, JoinedU, V: Monoid](
      source1: Producer[P, T1],
      source2: Producer[P, T2],
  ...
  def multipleSummerJob[P <: Platform[
    P
  ], T1, T2, K1, V1: Monoid, K2, V2: Monoid](
      source: Producer[P, T1],
...
class TestGraphs[P <: Platform[
  P
], T: Manifest: Arbitrary, K: Arbitrary, V: Arbitrary: Equiv: Monoid](
    platform: P
  ...
  def leftJoinChecker[U: Arbitrary, JoinedU: Arbitrary](
      ...
      postJoinFn: ((K, (U, Option[JoinedU]))) => List[(K, V)]
  ): Boolean = {
    val currentStore = store()

...
object Storm {
  ...
  def service[K, V](
      serv: => ReadableStore[K, V]
  ): ReadableServiceFactory[K, V] =
  ...
  implicit def spoutAsStormSource[T](
      spout: Spout[T]
...
abstract class Storm(
  ...
  private def scheduleSpout[K](
    ...
    val parallelism = getOrElse(
      stormDag,
      node,
      parOpt.getOrElse(DEFAULT_SOURCE_PARALLELISM)
    ).parHint

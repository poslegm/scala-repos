...
object Scalding {
  ...
  private def bisectingMinify(mode: Mode, desired: DateRange)(
    ...
    def isGood(end: Long): Boolean =
      STry(
        factory(DateRange(desired.start, RichDate(end))).validateTaps(mode)
      ).isSuccess
  ...
  /**
    ...
    * parsed before you can extract the timestamps.
    */
  def optionMappedPipeFactory[T, U](
      factory: (DateRange) => Mappable[T]
  ...
  private[scalding] def joinFP[T, U](
      left: FlowToPipe[T],
      right: FlowToPipe[U]
  ): FlowProducer[(TimedPipe[T], TimedPipe[U])] =
    for {
      t <- left
      u <- right
    } yield ((t, u))
  ...
  def toPipe[T](
      ...
      fd: FlowDef,
      mode: Mode
  ): Try[(DateRange, TypedPipe[(Timestamp, T)])] = {
...
class Memo[T] extends java.io.Serializable {
  ...
  def getOrElseUpdate(
      in: (FlowDef, Mode),
      rdr: Reader[(FlowDef, Mode), TimedPipe[T]]
  ): TimedPipe[T] =
...
class Scalding(
  ...
  def toFlow(
      ...
      mode: Mode,
      pf: PipeFactory[_]
  ): Try[(Interval[Timestamp], Option[Flow[_]])] = {

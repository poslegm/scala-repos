...
trait TypedPipe[+T] extends Serializable {
  ...
  def writeThrough[U >: T](
      dest: TypedSink[T] with TypedSource[U]
  ...
  /**
    ...
    */
  def make[U >: T](
      dest: Source with TypedSink[T] with TypedSource[U]
  ): Execution[TypedPipe[U]] =
  ...
  def hashLookup[K >: T, V](
      grouped: HashJoinable[K, V]
  ): TypedPipe[(K, Option[V])] =
  ...
  /**
    ...
    */
  def addTrap[U >: T](
      trapSink: Source with TypedSink[T]
...
class TypedPipeFactory[T] private (
  ...
  private def unwrap(pipe: TypedPipe[T], st: Array[StackTraceElement])(
      implicit flowDef: FlowDef,
      mode: Mode
  ): (TypedPipe[T], Array[StackTraceElement]) = pipe match {

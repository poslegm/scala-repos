...
sealed trait ReduceStep[K, V1] extends KeyedPipe[K] {
  ...
  protected def groupOp[V2](
      gb: GroupBuilder => GroupBuilder
  ): TypedPipe[(K, V2)] =
    groupOpWithValueSort[V2](None)(gb)

  protected def groupOpWithValueSort[V2](
      valueSort: Option[Ordering[_ >: V1]]
...
case class UnsortedIdentityReduce[K, V1](
  ...
  override def bufferedTake(n: Int) =
    if (n < 1) {
      ...
      val pretake: TypedPipe[(K, V1)] =
        mapped.mapValues { v: V1 => mon.build(v) }.sumByLocalKeys.flatMap {

...
sealed trait Matrix2[R, C, V] extends Serializable {
  ...
  def rowL2Normalize(implicit
    ...
    lazy val result = MatrixLiteral(
      ...
      this.sizeHint
    ).sumColVectors.toTypedPipe.map {
      case (r, c, x) => (r, r, 1 / scala.math.sqrt(x))
    } // diagonal + inverse
    MatrixLiteral(
      result,
      SizeHint.asDiagonal(this.sizeHint.setRowsToCols)
    ) * matD
  }
...
case class Product[R, C, C2, V](
  ...
  private def computePipe(
      joined: TypedPipe[(R, C2, V)] = toOuterSum
  ): TypedPipe[(R, C2, V)] = {
    if (isSpecialCase) {
      joined
    } else {

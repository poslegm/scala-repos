...
final class AssessApi(
  ...
  def getGameResultsById(gameId: String) =
    getResultsByGameIdAndColor(
      gameId,
      Color.White
    ) zip getResultsByGameIdAndColor(gameId, Color.Black) map { a =>
  ...
  def getPlayerAggregateAssessment(
    ...
    UserRepo.byId(userId) zip getPlayerAssessmentsByUserId(
      userId,
      nb
    ) zip relatedUsers zip
  ...
  def onAnalysisReady(
    ...
    shouldAssess.?? {
      ...
      createPlayerAssessment(
        assessible playerAssessment chess.White

...
private[tournament] final class TournamentApi(
            ...
            case pairings =>
              pairings.map { pairing =>
                PairingRepo
                  ...
                  game => sendTo(tour.id, StartGame(game))
                }
  ...
  private def featureOneOf(
    ...
    tour.featuredId.ifTrue(
      pairings.nonEmpty
    ) ?? PairingRepo.byId map2 RankedPairing(ranking) map (_.flatten) flatMap {
      curOption =>
        pairings
  ...
  def wipe(tour: Tournament): Funit =
    TournamentRepo.remove(tour).void >> PairingRepo.removeByTour(
      tour.id
    ) >> PlayerRepo
    ...
    Sequencing(tourId)(TournamentRepo.enterableById) { tour =>
      PlayerRepo.join(tour.id, me, tour.perfLens) >> updateNbPlayers(
        ...
        if (!tour.`private`)
          timeline !
            (Propagate(
              TourJoin(me.id, tour.id, tour.fullName)
            ) toFollowersOf me.id)
      }
    }
  }
  ...
  def fetchVisibleTournaments: Fu[VisibleTournaments] =
    TournamentRepo.publicCreatedSorted(
      6 * 60
    ) zip TournamentRepo.publicStarted zip TournamentRepo

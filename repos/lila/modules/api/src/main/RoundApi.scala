...
private[api] final class RoundApi(
  ...
  def player(pov: Pov, apiVersion: Int)(implicit ctx: Context): Fu[JsObject] =
    GameRepo.initialFen(pov.game) flatMap { initialFen =>
      jsonView.playerJson(
      ...
      ) zip getTourAndRanks(pov.game) zip (pov.game.simulId ?? getSimul) zip
        ...
        case ((((json, tourOption), simulOption), note), forecast) =>
          (blindMode _ compose withTournament(
            pov,
            tourOption
          ) _ compose withSimul(pov, simulOption) _ compose withSteps(
            pov,
            none,
            initialFen,
            withOpening = false
          ) _ compose withNote(note) _ compose withBookmark(ctx.me ?? {
  ...
  def watcher(
    ...
    initialFenO.fold(GameRepo initialFen pov.game)(fuccess) flatMap {
      initialFen =>
        jsonView.watcherJson(
        ...
        ) zip getTourAndRanks(pov.game) zip
          ...
          case (((json, tourOption), simulOption), note) =>
            (blindMode _ compose withTournament(
              pov,
              tourOption
            ) _ compose withSimul(pov, simulOption) _ compose withNote(
              note
            ) _ compose withBookmark(ctx.me ?? {
              bookmarkApi.bookmarked(pov.game, _)
            }) _ compose withSteps(
              pov,
              analysis,
            ...
            ) _ compose withAnalysis(analysis) _)(json)
        }
    }
  ...
  def userAnalysisJson(
    ...
    owner.??(forecastApi loadForDisplay pov).flatMap { fco =>
      jsonView
        ...
        initialFen,
        withOpening = true
      ) _ map withForecast(pov, owner, fco) _
    }

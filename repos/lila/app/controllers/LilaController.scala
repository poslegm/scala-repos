...
private[controllers] trait LilaController
  ...
  protected def AuthBody(
      f: BodyContext[_] => UserModel => Fu[Result]
  ): Action[AnyContent] =
  ...
  protected def Secure(
      perm: Permission.type => Permission
  ...
  protected def NoEngine[A <: Result](
      a: => Fu[A]
  )(implicit ctx: Context): Fu[Result] =
  ...
  protected def NoPlaybanOrCurrent(
      a: => Fu[Result]
  )(implicit ctx: Context): Fu[Result] =
  ...
  protected def OptionFuResult[A](
      fua: Fu[Option[A]]

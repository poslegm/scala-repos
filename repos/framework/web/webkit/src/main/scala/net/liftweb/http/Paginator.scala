...
trait PaginatorSnippet[T] extends Paginator[T] {
  ...
  def paginate: CssSel = {
    ...
    ".first *" #> pageXml(0, firstXml) & ".prev *" #> pageXml(
      ...
      0 until numPages
    ) _ & ".zoomed-pages *" #> pagesXml(zoomedPages) _ & ".next *" #> pageXml(
      max(0, min(first + itemsPerPage, itemsPerPage * (numPages - 1))),
      nextXml
    ) & ".last *" #> pageXml(
      itemsPerPage * (numPages - 1),
      lastXml
    ) & ".records *" #> currentXml & ".records-start *" #> recordsFrom & ".records-end *" #> recordsTo & ".records-count *" #> count
  }
}

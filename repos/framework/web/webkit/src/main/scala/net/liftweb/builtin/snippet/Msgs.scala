...
object Msgs extends DispatchSnippet {
  ...
  def render(styles: NodeSeq): NodeSeq = {
    ...
    (<div>{
      renderNotices()
    }</div> % ("id" -> LiftRules.noticesContainerId)) ++ noticesFadeOut(
      NoticeType.Notice
    ) ++ noticesFadeOut(NoticeType.Warning) ++ noticesFadeOut(
      NoticeType.Error
    ) ++ effects(NoticeType.Notice) ++ effects(NoticeType.Warning) ++ effects(
      NoticeType.Error
    )
  }
